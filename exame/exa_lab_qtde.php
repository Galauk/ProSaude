...
$sql = pg_query("select *from medico where prestador_servico ='L' order by med_nome");
#and med_codigo = 2165 
...
$query = pg_query("select TRANSLATE(proc_nome, 'ZZZ-', '') as newprocnome,*from procedimento as p join tipodeexame as t on p.proc_codigo = t.proc_codigo order by TRANSLATE(proc_nome, 'ZZZ-', '')");
...
echo "select TRANSLATE(proc_nome, 'ZZZ-', '') as newprocnome,grm.graex_qtde as graex_qtde,* from grade_exame as grm left join procedimento as proc on proc.proc_codigo = grm.proc_codigo where grm.gex_codigo = '$gex_codigo' and grm.med_codigo = '$med_codigo' and grm.graex_data = '$gex_periodo' order by TRANSLATE(proc_nome, 'ZZZ-', '')";
  while($row = pg_fetch_array($sql)) {
...
$q = "BEGIN; ";
     ...
     $conta=1;
               ...
               while ($conta <= $qtde){
            	 ...
            	 /*$mesAno = explode("/", $dataini);
				 $condicao = $mesAno[1]."/".$mesAno[2];
				 $pegaUltimoPeriodo = pg_query("SELECT * FROM grade_exame_mensal WHERE gex_periodo = '$gex_periodo'");
				 $regUltimoPeriodo = pg_fetch_array($pegaUltimoPeriodo);
				 $regUltimoPeriodo["gex_codigo"];*/
				 
				 $sqlGradeUnidade = "SELECT * 
				  						FROM grade_exame_unidade 
				  					   WHERE gex_codigo = $gex_codigo
				  					     AND proc_codigo = $rr[proc_codigo]";
				 $queryGradeUnidade = pg_query($sqlGradeUnidade);
				 $regGradeUnidade = pg_fetch_array($queryGradeUnidade);
				 include "../ajax/update/agendamento/grade_exame_unidade_ajax.php";
}
	//exit();
               echo "<SCRIPT LANGUAGE=\"JavaScript\">";//"
