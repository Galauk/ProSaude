...
function desativa()
{
  document.getElementById('consolidall').src = '<?= $_SESSION[linkroot].$_SESSION[comum];?>imgs/confirm_recep_todos_off.jpg';
  document.getElementById('consolidcam').href = '#';
  document.getElementById('consolidcam').onclick = function() {
    alert("Ja foi gerado um item de movimentacao para esta requisicao. Voce deve fazer a confirmacao individualmente.");
    return false;
  }
}
...
$sql = pg_query("select set_codigo,set_nome from setor where set_estoque = 'S'
                            and set_distribuidor = 'S' $decisao");
...
reglog($id_login,"Acessando Confirmacao do Recebimento da Requisicao de  Materiais");
echo "<fieldset><legend>CONFIRMACAO DA REQUISICAO DE MATERIAIS</legend>";
		...
		<td>
		  <fieldset>
			...
			<table width=100% align=center cellspacing=3 cellpadding=0 border=0>
			  <tr>
				<form method=post action='confirmacao_materiais.php?id_login=$id_login'>
				<input type=hidden name=acao value=busca>
				<input type=hidden name=id_login value=$id_login>
				<td width=180 align=right>Buscar </td>";
$sql = pg_query("select set_codigo,set_nome
		                 from setor 
				 where set_codigo in (select set_codigo from usuarios_setores
				                      where usr_codigo = $id_login)");				

echo"<td width=90><select name=palavra_chave class='box'>";
...
echo "
				...
				<td>".ChmodBtn($id_login,'procurar','confirmacao_materiais.php')."</td></form>
...
if (chmodbtn($id_login,"listar_if","confirmacao_materiais.php"))
{
	...
	$sql="select distinct a.data, a.req_data, a.req_nr_nota, a.setor, a.codsetor,
		...
		a.req_codigo, 'R' as tipo
		from req_dispensado a, setor b, requisicao c 
		WHERE a.req_codigo = c.req_codigo 
		AND b.set_codigo = c.set_entrada 
		and (c.set_entrada = $setortemp or c.set_saida = $setortemp) 
		and set_entrada in (select set_codigo from setor
		                           where usr_codigo = $id_login) 
		
	        UNION
		select distinct a.data, a.req_data, a.req_nr_nota, a.setor, a.codsetor, 
		a.codsetorsolicit, a.nomesetorsolicit, a.desc_movimentacao, a.req_tipo, a.operacao, 
		a.req_codigo, 'T' as tipo
		from reqtransf_dispensado a, setor b, requisicao c 
		WHERE a.req_codigo = c.req_codigo 
		AND b.set_codigo = c.set_entrada 
	...
	while($row=pg_fetch_array($sql)) {
		...
		ChmodBtn($id_login,'editar','confirmacao_materiais.php?acao=form_edit&req_codigo='.$row[req_codigo])."&nbsp;
...
if($acao=="busca")
{//echo"<pre>".print_r($_GET)."</pre>";
reglog($id_login,"Buscando em Confirmacao do Recebimento da Requisicao de  Materiais $palavra_chave");
//
//-> Verificando Busca
...
$uni[0]=="" ? $decisao = "" : $decisao = " AND setor.uni_codigo = ".$uni[0];
$sql = pg_query("select set_codigo,set_nome from setor
				 where   set_codigo in (select set_codigo from usuarios_setores
				                      where usr_codigo = $id_login)");
...
$sql="select distinct a.data, a.req_data, a.req_nr_nota, a.setor, a.codsetor,
	  ...
	  from req_dispensado a, setor b, requisicao c 
          ...
          and (c.set_entrada in (select set_codigo from usuarios_setores
				                      where usr_codigo = $id_login))
          and (c.set_entrada = $palavra_chave)
	  union
          select distinct a.data, a.req_data, a.req_nr_nota, a.setor, a.codsetor, 
          a.codsetorsolicit, a.nomesetorsolicit, a.desc_movimentacao, a.req_tipo, a.operacao, 
          a.req_codigo, 'T' as tipo
...
while($row=pg_fetch_array($sql)) {
	echo "<tr>
	       ...
	       <td align=center style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9;'>$row[setor]</td>
	       <td align=center style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9;'>$row[nomesetorsolicit]</td>
	       <td width=60 style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9;'>".ChmodBtn($id_login,'editar','confirmacao_materiais.php?acao=form_edit&req_codigo='.$row[req_codigo])."&nbsp;<img src='".$_SESSION[linkroot].$_SESSION[comum]."imgs/imprimir.jpg' onclick='imprimi(".$row[req_codigo].")'></td>
	     </tr>";
}
echo "</tr>
	     </table>
...
echo "
<script type='text/javascript'>
    var acao = \"document.location.href = 'confirmacao_materiais.php?acao=form_consolid&id_login=$id_login'\";
    setTimeout( acao, 1000 * 60 );
</script>
";
}
...
if($acao=="form_edit") {
	//echo"<pre>".print_r($_GET)."</pre>";
	reglog($id_login,"Formulario de Edicao de Confirmacao do Recebimento da Requisicao de  Materiais");
	//
          ...
          <td>
           <fieldset>
            <legend>Opções de Cadastro</legend>
             <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
              <tr>
               <td width=79><a href=confirmacao_materiais.php?id_login=$id_login><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/voltar_on.gif border=0></a></td>
               <td width=79><a id='consolidcam' href='confirmacao_materiais.php?id_login=$id_login&acao=todos&req_codigo=$req_codigo'><img id='consolidall' src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/confirm_recep_todos_on.jpg alt='Confirmar Todos' border=0></a></td>
               <td>&nbsp;</td>
              </tr>
             </table>
           </fieldset>
	...
	/*$sql = "select rd.pro_codigo, pro_nome, coalesce(ireq_quantidade,0) as ireq_quantidade,
	 coalesce(ireq_qtde_solicitada,0) as ireq_qtde_solicitada,
	 sum(sal_qtde) as estoqueatual,
	 ireq_codigo, desc_status, req_tipo
	 from req_dispensado rd join saldo s
	 on s.pro_codigo = rd.pro_codigo
	 where  req_codigo = '$req_codigo'
	 and    ireq_quantidade > 0
	 and    ireq_quantidade is not null
	 group by  rd.pro_codigo,
	 pro_nome,
	 ireq_quantidade,
	 ...
	 req_tipo
	 UNION
	 select rd.pro_codigo, pro_nome, coalesce(ireq_quantidade,0) as ireq_quantidade,
	 coalesce(ireq_qtde_solicitada,0) as ireq_qtde_solicitada,
	 sum(sal_qtde) as estoqueatual,
	 ireq_codigo, desc_status, req_tipo
	 from reqtransf_dispensado rd join saldo s
	 on s.pro_codigo = rd.pro_codigo
	 where  req_codigo = '$req_codigo'
	 and    ireq_quantidade is not null
	 and    ireq_quantidade > 0
	...
	<table width=98% align=center cellspacing=0 cellpadding=0 border=0>
	 <tr>
	  <td>
	   <fieldset>
	    <legend>Confirmacao do Recebimento da Requisicao de  Materiais de Materiais</legend>
	    <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
	...
	while($row=pg_fetch_array($sqlmovimento)) {
		...
		//Rotina que verifica se ja tem algum item gerado individualmente pelo sistema e nao permite gerar todos os itens
		//ao mesmo tempo na gravacao da movimentacao.
		$sqlvemov = "select count(req_codigo) from movimento
		 where  req_codigo = $req_codigo";
		$sqlvemovimento =  pg_fetch_array(pg_query($sqlvemov));
		...
		if ($row[7] == 'T')   $produto = $row[1] . " Lote: " . $lotevalid[0] . " Valid: " . $lotevalid[1];
		if ($row[7] == 'S')   $produto = $row[1];
		echo "
	       <tr>
	          <td align=left style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9;'>$produto</td>
	          ...
	          <td align=center style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9; '>$row[desc_status]</td>		  
		
	       </tr>";
		/*if (($row['ireq_quantidade'] <= $row['estoqueatual'] && $row['ireq_quantidade'] > 0)) {
		 echo "
		 <a href=$PHP_SELF?id_login=$id_login&acao=edit&ireq_codigo=$row[ireq_codigo]&req_codigo=$req_codigo><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/confirm_recep_on.jpg alt='Confirmar Recep&ccedil;&atilde;o' border=0></a> &nbsp; &nbsp;";
		 }*/
		/*       echo "
		 <a href=$PHP_SELF?id_login=$id_login&acao=form_altera_item&ireq_codigo=$row[ireq_codigo]&req_codigo=$req_codigo><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/editar_on.jpg border=0></a></td>
		 </tr>";*/
	}
	echo"
	     </table>
	  ...
	  <td>
	   <fieldset>
		...
		<td width=20>Produto:</td>
	     ...
	     <tr>
	       <td width=79><a href=confirmacao_materiais.php?id_login=$id_login&req_codigo=$req_codigo&ireq_codigo=$ireq_codigo&acao=form_edit><input type=image  src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/voltar_on.gif border=0></a></td>
...
if($acao=="edit") {
	...
	/* $sql .= "UPDATE itens_requisicao set " .
	 "ireq_consolidado = 'A'," .
	 ($id_login ? "usr_codigo='$id_login'" : "usr_codigo=null") . "  " .
	 "WHERE ireq_codigo ='$ireq_codigo'; ";*/
	...
	reglog($id_login,"Editando Confirmacao do Recebimento da Requisicao de  Materiais $ireq_codigo");
	...
	if($psql)
	{
		...
		echo "<SCRIPT LANGUAGE=\"JavaScript\">
                        setTimeout(\"location='confirmacao_materiais.php?id_login=$id_login&req_codigo=$req_codigo&acao=form_edit'\", 2000);
                </SCRIPT>";
	} else {
		echo "<br><br><br><br><br><br><br><br><br><br><br><br><br>
                        <table height=100 width=50% align=center cellspacing=0 cellpadding=0 border=0 style='border-top:1px solid;border-bottom:1px solid;border-color:909090;'>
...
if($acao=="edit_item") {
	...
	($id_login ? "usr_codigo='$id_login'" : "usr_codigo=null") . "  " .
                  "where ireq_codigo ='$ireq_codigo'");
	reglog($id_login,"Alterando quantidade do produto na Confirmacao do Recebimento da Requisicao de  Materiais $ireq_codigo");
	//msg($id_login."&acao=form_edit&req_codigo=".$req_codigo,$acao,$sql);
	if($sql)
	{
		echo "<br><br><br><br><br><br><br><br><br><br><br><br><br>
	...
	//confirmacao_materiais.php?id_login=$id_login'
}
...
if($acao=="todos") {
	...
	/* $sqlmovimento =pg_query("select s.pro_codigo,
	 pro_nome,
	...
	/* $sql .= " INSERT INTO itens_movimento
	 ...
	 ($codmov, $row[0], $id_login, 'S', '$row[4]', $row[2], $row[3]); ";
	...
	$sqlQuery="SELECT *
						  FROM movimento AS m 
						  JOIN requisicao AS r
						    ON m.req_codigo = r.req_codigo
						  JOIN itens_requisicao ir
						    ON r.req_codigo = ir.req_codigo 
					     WHERE  r.req_codigo= $req_codigo";
	/*$sqlQuety="SELECT *			     FROM itens_requisicao ir
				     JOIN requisicao r
				       ON r.req_codigo = ir.req_codigo
		            WHERE
			            r.req_data > '2011-03-13'
	                  AND ir.ireq_consolidado != 'A' 
	                  AND req_tipo !='T'";*/
	$queryIreq = pg_query($sqlQuery);
	...
	$sql = "begin; ";
	while ($resIreq = pg_fetch_array($queryIreq)){
		$pegaSql="update itens_requisicao set " .
                  "ireq_consolidado = 'A'," .
		($id_login ? "usr_codigo='$id_login'" : "usr_codigo=null") . "  " .
                  "where ireq_codigo ='$resIreq[ireq_codigo]'";
		
		$update = pg_query($pegaSql);
	...
	if($psql)
	{
		...
		echo "<SCRIPT LANGUAGE=\"JavaScript\">
                        setTimeout(\"location='confirmacao_materiais.php?id_login=$id_login&req_codigo=$req_codigo'\", 2000);
                </SCRIPT>";
	} else {
		echo "<br><br><br><br><br><br><br><br><br><br><br><br><br>
                        <table height=100 width=50% align=center cellspacing=0 cellpadding=0 border=0 style='border-top:1px solid;border-bottom:1px solid;border-color:909090;'>
	...
	/*if($psql)
	 ...
	 echo "<SCRIPT LANGUAGE=\"JavaScript\">
	 setTimeout(\"location='confirmacao_materiais.php'\", 2000);
	 </SCRIPT>";
	 } else {
	 echo "<br><br><br><br><br><br><br><br><br><br><br><br><br>
	 <table height=100 width=50% align=center cellspacing=0 cellpadding=0 border=0 style='border-top:1px solid;border-bottom:1px solid;border-color:909090;'>
