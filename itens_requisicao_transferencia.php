	  ...
	  <td>
	   <fieldset>
		...
		$sql = "select * from produto
				...
				AND   pro_tipo = 'M'
				AND   pro_situacao = 'A'
				AND   pro_codigo in (select pro_codigo from saldo where sal_qtde > 0)				
				order by pro_nome";
		echo $sql;	
		$query = db_query($sql);
	   ...
	   echo "</select>
		   ...
		   $sel = "SELECT req_data
			...
			$exec_sel = pg_query($sel);
				...
				echo "<input type=image src='".$_SESSION[linkroot].$_SESSION[comum]."imgs/add_on.gif' alt='Adicionar'>";
		
			echo "</td>
	       <td width=79>";
			
				echo "<a href=requisicao_transferencia.php?id_login=$id_login&requis=$req_codigo&final=1><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/finalizar_on.jpg alt='Finalizar' border=0></a>";
			
				echo "&nbsp;";
			
		   echo "</td>
           <td width=79>";
				echo "<a href=requisicao_transferencia.php?id_login=$id_login&req_codigo=$req_codigo&link_extra=$link_extra><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/voltar_on.gif alt='Voltar' border=0></a>";
			echo "</td>
	      </tr></form>
	     </table>
	   </fieldset>
	   ...
	   echo "</select>
	     ...
	     <tr>
	       <td width=95><input type=image src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/editar_on.jpg></td>
               <td width=79><a href=itens_requisicao_transferencia.php?id_login=$id_login&req_codigo=$req_codigo&action=form_inclui_item><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/voltar_on.gif border=0></a></td>
	      </tr></form>
	     </table>
	   </fieldset>
	  </td>
 ...
 if($action=="form_insert") {
    ...
    $buscaValidade = "SELECT to_char(sal_validade,'dd/mm/yyyy') as datavalidade,* 
							   ...
							   WHERE pro_codigo = $pro_codigo 
								 ...
								 AND sal_qtde > 0
								 AND set_codigo= $rowmovim[set_saida]
    ...
    $insert = "insert into itens_requisicao ( pro_codigo,
    													  ireq_quantidade,
    													  ireq_qtde_solicitada,
    													  ireq_vlrunit,
    													  req_codigo,
    													  ireq_consolidado) values ( " .
