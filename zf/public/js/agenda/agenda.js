...
function afterSubmit(json){	
	...
	if(!json.success){
		...
		$("#mensagem-dialog").dialog({
			...
			height: 180,
			close: function(){
				window.location.href = baseUrl + "/agenda/agenda/";
				$(this).remove();
			},
			buttons: {
				Sim: function(){
...
function marcarDia(){
	...
	if($(this).hasClass("com-vaga")){
        ...
        mensagemSemOk("carregando-conis", "Aguarde", "Carregando vagas disponíveis...", 290, 100);
        $.get(baseUrl+"/agenda/convenio-itens/get-vagas-turno", {data, coni}, res => {
            
            let vm = ''
            let vt = ''

            if((res.total.max_dia_manha == null) && (res.total.max_dia_tarde == null)){
                $("#div_per").hide()
                return false
            }
            ...
            $("#div_per input").prop('checked', false)
            
            if((res.total.max_dia_manha - res.manha) === 0) {
                vm = "(Sem vagas)"
                
                $('#per_m').on('click', function(){
                    $('#div-modal').append('Deseja fazer um agendamento extra?')
                    .dialog(
                        {
                            modal: true,
                            width: 250,
                            ...
                            buttons: {
                                Sim: function(){
                                    $(this).dialog('close')
                                    $("#div_per input#per_m").prop('checked', true)
                                },
                                Não: function(){
                                    $(this).dialog('close')
                                    $("#div_per input#per_m").prop('checked', false)
                                }
                            }
                        }
                    )
                })
            } else if((res.total.max_dia_manha - res.manha) > 1 ){
                $('#per_m').off('click')
                vm = "("+(res.total.max_dia_manha - res.manha)+" vagas)"
            } else if((res.total.max_dia_manha - res.manha) < 0 ){
                $('#per_m').off('click')
                let negativo = (res.total.max_dia_manha - res.manha) * -1
                vm = "Sem vagas <font color='red'>("+negativo+" extra)</font>"
            } else {
                $('#per_m').off('click')
                vm = "("+(res.total.max_dia_manha - res.manha)+" vaga)"
            }
            
            if((res.total.max_dia_tarde - res.tarde) === 0) {
                vt = "(Sem vagas)"
                $('#per_t').on('click', function(){
                    $('#div-modal').append('Deseja fazer um agendamento extra?')
                    .dialog(
                        {
                            modal: true,
                            ...
                            buttons: {
                                Sim: function(){
                                    $("#div_per input#per_t").prop('checked', true)
                                    $(this).dialog('close')
                                },
                                Não: function(){
                                    $("#div_per input#per_t").prop('checked', false)
                                    $(this).dialog('close')
                                }
                            }
                        }
            ...
            } else if((res.total.max_dia_tarde - res.tarde) > 1 ){
                $('#per_t').off('click')
                vt = "("+(res.total.max_dia_tarde - res.tarde)+" vagas)"
            } else if((res.total.max_dia_tarde - res.tarde) < 0 ){
                $('#per_t').off('click')
                let neg = (res.total.max_dia_tarde - res.tarde) * -1
                vt = "Sem vagas <font color='red'>("+neg+" extra)</font>"
            } else {
                $('#per_t').off('click')
                vt = "("+(res.total.max_dia_tarde - res.tarde)+" vaga)"
            }

            $("#p1 b").text(res.total.max_dia_manha_horario || '07:00')
            $("#p2 b").text(res.total.max_dia_tarde_horario || '14:00')
		...
		$("[data-coni="+coni+"]").html("&nbsp;"); // limpa todos da mesma lin)ha
...
function habilitarOuNaoBotaoSalvar(){
    ...
    if($("#div_per").is(":visible")){
        if($("#div_per input").is(":checked")){
            tudo_ok = true
        } else {
            tudo_ok = false
        }
