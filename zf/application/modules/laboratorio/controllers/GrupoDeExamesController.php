...
class Laboratorio_GrupoDeExamesController extends Zend_Controller_Action {
    ...
    public function grupoPrenatalAction(){
        ...
        $this->_helper->layout->setLayout("grupo-de-exames/grupo-prenatal");
        ...
        if ($codGrupo) {
            //print_r($tbGruex->getGruposPorId($codGrupo)->toArray()); die();
            $this->view->grupo_prenatal = $tbGruex->getGruposPorId($codGrupo)->toArray();
            $this->view->procedimentos = $tbGruProc->listaDadosConfigGrupoDeExames($codGrupo)->toArray();
        }
        if($inserir){
