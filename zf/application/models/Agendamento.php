...
class Application_Model_Agendamento extends Elotech_Db_Table_Abstract {
   ...
   // protected $_dependentTables = array('Atendimento');
    ...
    public function getAgendaMedico() {
        ...
        ->setIntegrityCheck(FALSE)
        ->from(array("age" => "agendamento"), array("minutos"=>"(SELECT EXTRACT(MINUTE FROM (to_timestamp(current_date || ' ' || current_time, 'YYYY-MM-DD HH24:MI')-to_char(age_data_atend, 'YYYY-MM-DD HH24:MI')::TIMESTAMP)))","age.age_codigo","age_data_atend","age_horario as age_hora","age_atendido","med_codigo", "age_estratificacao_risco","horas"=>"(SELECT EXTRACT(HOURS FROM (to_timestamp(current_date || ' ' || current_time, 'YYYY-MM-DD HH24:MI')-to_char(age_data_atend, 'YYYY-MM-DD HH24:MI')::TIMESTAMP)))"))
        ...
        ->joinleft(array("est"=>"estratificacao"),"age.est_codigo = est.est_codigo","est_nome")
        ...
        ->where("age.med_codigo=$usr->usr_codigo  OR age.med_codigo = 99999")
                        ->where("age(now(), to_timestamp(age_data || ' ' || age_horario, 'YYYY-MM-DD HH24:MI')) < interval '$tempo hour'")
                        ->order("age.age_data_atend asc")
	...
	//	die("asdfasdf");
        ...
        $wher1 = $this->select(FALSE)
                ->setIntegrityCheck(FALSE)
                ->from(array("age" => "agendamento"), array("minutos"=>"(SELECT EXTRACT(MINUTE FROM (to_timestamp(current_date || ' ' || current_time, 'YYYY-MM-DD HH24:MI')-to_char(age_data_atend, 'YYYY-MM-DD HH24:MI')::TIMESTAMP)))","age.age_codigo","age_horario as age_hora","age_atendido","med_codigo","age_data_atend","age_ordem","age_horario","age_estratificacao_risco","horas"=>"(SELECT EXTRACT(HOURS FROM (to_timestamp(current_date || ' ' || current_time, 'YYYY-MM-DD HH24:MI')-to_char(age_data_atend, 'YYYY-MM-DD HH24:MI')::TIMESTAMP)))"))
                ->join(array("usu" => "usuario"), "usu.usu_codigo=age.usu_codigo", array("usu_codigo", "usu_nome", "usu_mae", "usu_end_cidade", "usu_datanasc","usu_prontuario"))
        ...
        $where = $this->select(FALSE)
                        ...
                        ->order(array("age_data_atend asc","age_ordem","age_codigo"));
...
//        }
    ...
    public function getPacientesAgendados($uni_codigo=FALSE,$usr_codigo=FALSE,$esp_codigo=FALSE,$age_data=FALSE,$pac=FALSE){
        $where = $this->select()
                      ->setIntegrityCheck(FALSE)
                      ->from(array("age"=>"agendamento"),array("age_encaixe","age_ordem","age_codigo","age_horario","age_atendido" => "(CASE WHEN age_atendido='P' THEN 'Pré-Consulta' WHEN age_atendido='S' THEN 'Recepcionado' WHEN age_atendido='A' THEN 'Atendido' WHEN age_atendido='N' THEN 'Agendado' WHEN age_atendido='T' THEN 'Transferido' WHEN age_atendido='F' THEN 'Faltou' WHEN age_atendido='E' THEN 'Em Atendimento' WHEN age_atendido='I' THEN 'Atendimento Incluso' WHEN age_atendido='M' THEN 'Falta Médica' END)","cor" => "(CASE WHEN age_atendido='S' THEN 'blue' WHEN age_atendido='A' THEN '#148e00' WHEN age_atendido='N' THEN '#2e6e9e' END)","age_atendido AS status"))
                      ->join(array("coni"=>"convenio_itens"),"coni.coni_codigo = age.coni_codigo","")
                      ->join(array("conv"=>"convenio"),"coni.conv_codigo = conv.conv_codigo","")
                      ->joinleft(array("est"=>"estratificacao"),"age.est_codigo = est.est_codigo","est_nome")
                      ...
                      if(is_array($usr_codigo)){
                        $where->where('coni.usr_codigo IN (?)', $usr_codigo)
                        ->join(array('usr'=>'usuarios'), 'coni.usr_codigo = usr.usr_codigo', array('usr_nome'));
                      } else {
                        $where->where("coni.usr_codigo='$usr_codigo'");
                      }
                      if($esp_codigo){
                        $where->where("age.esp_codigo='$esp_codigo'");
                      }
                      $where->where("age.age_data='$age_data'")
                      //->where("age.age_atendido ='N'")
                      ->order("age_ordem");
					  $pac ? $where->order(array("age.age_atendido DESC")) : $where->order(array("age.age_horario"))
                      ->order("age.age_horario");
    ...
    public function getAgendamento($age_codigo=FALSE){
        $where = $this->select(FALSE)
            ...
            ->join(array("usu"=>"usuario"),"usu.usu_codigo=age.usu_codigo", array('usu_nome', 'usu_datanasc', 'usu_cartao_sus', 'usu_mae'))
            ->joinLeft(array("dom" => "domicilio"),"dom.dom_codigo=usu.dom_codigo", array('dom_codigo','dom_data_cadastro','rua_codigo','dom_numero','dom_segmento','dom_complemento','co_tipo_domicilio','dom_telefone','usu_codigo_responsavel','dom_ponto_referencia','bai_codigo','dom_as_localizacao','dom_as_abrigo','usr_codigo','cod_equipe'))
...
/**
    ...
    public function getTotalAgendamento($data_ini=false,$data_fim=false,$ine=false,$uni_codigo=false){
        $where = $this->select(FALSE)
                        ->setIntegrityCheck(FALSE) 
                        ->from(array("age"=>"agendamento"),array("count(*) total"))
                        ...
                        if($data_ini){
                         $where->where("age_data >= ?",$data_ini);
                        }
                        if($data_fim){
                         $where->where("age_data <= ?",$data_fim);
                        }
                        if($ine){
    ...
    public function getTotalConsultaAgendada($data_ini=false,$data_fim=false,$ine=false,$uni_codigo=false){
        $where = $this->select(FALSE)
                        ->setIntegrityCheck(FALSE) 
                        ->from(array("age"=>"agendamento"),array("count(*) total"))
                        ->join(array("ate" => "atendimento"),"age.age_codigo = ate.age_codigo","")
    ...
    public function getTotalDemanda($data_ini=false,$data_fim=false,$ine=false,$uni_codigo=false){
        $where = $this->select(FALSE)
                        ->setIntegrityCheck(FALSE) 
                        ->from(array("age"=>"agendamento"),array("count(*) total"))
                        ->join(array("ate" => "atendimento"),"age.age_codigo = ate.age_codigo","")
    ...
    public function getTotalEncaminhamento($data_ini=false,$data_fim=false,$ine=false){
        $where = $this->select(FALSE)
                        ->setIntegrityCheck(FALSE) 
                        ->from(array("age"=>"agendamento"),array("count(*) total"))
                        ->join(array("ate" => "atendimento"),"age.age_codigo = ate.age_codigo","")
                        ->join(array("enc" => "encaminhamento"),"ate.ate_codigo = enc.ate_codigo","")
                        ->joinLeft(array("ue" =>"usuarios_equipe"),"ue.usr_codigo = ate.med_codigo","")
                        ->joinLeft(array("ine" => "tb_equipe"), "ue.co_equipe = ine.co_seq_equipe","");
                        if($data_ini){
                         $where->where("age_data >= ?",$data_ini);
...
//      die($where);
    ...
    public function getDadosPorAtendimento($ate_codigo=FALSE){
         $where = $this->select(FALSE)
                        ->setIntegrityCheck(FALSE) 
                        ->from(array("age"=>"agendamento"),array("tat_codigo", "tp_cod"))
                        ->join(array("ate" => "atendimento"),"age.age_codigo = ate.age_codigo","")
