<?php
	...
	$dbconn3 = pg_connect("host=localhost port=5432 dbname=dbsocial_floresta_script user=postgres password=123");
    ...
    $sql = "SELECT DISTINCT 
			 ite.pro_codigo,
			 set.set_codigo,
			 ite.ite_lote AS sal_lote,
			 ite.ite_validade AS sal_validade
			FROM 
			 movimento AS mov
			INNER JOIN 
    ...
    while ($row = pg_fetch_array($query)) {
		...
		if ($diferenca != 0) {
			$novoSaldo = $saldoMov;
			/*
			$sqlConfSaldo = "SELECT * FROM saldo WHERE pro_codigo = '".$row["pro_codigo"]."' AND set_codigo = '".$row["set_codigo"]."' AND sal_lote = '".$row["sal_lote"]."' AND sal_validade = '".$row["sal_validade"]."'";
			$queryConfSaldo = pg_query($sqlConfSaldo);
			$numConfSaldo = pg_num_rows($queryConfSaldo);
			
			if ($numConfSaldo == 0) {
				$sqlInsert = "INSERT INTO saldo 
										(pro_codigo,set_codigo,sal_data,sal_qtde,sal_lote,sal_validade)
									VALUES
										('".$row["pro_codigo"]."','".$row["set_codigo"]."','NOW()','".$novoSaldo."','".$row["sal_lote"]."','".$row["sal_validade"]."')";
				$queryInsert = pg_query($sqlInsert);
			} else {
				$rowSaldo = pg_fetch_array($queryConfSaldo);
