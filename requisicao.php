...
function abre_movim(id_login, req_nr_nota)
{
    ...
    var url = './relatorio/ReqExibir.php?acao=form_edit&id_login='+id_login+'&req_nr_nota='+req_nr_nota;
    window.open(url,null,"height=400,width=750");
}
...
echo "<fieldset><legend>REQUISICAO DE MATERIAIS</legend>";
...
if (empty($acao) || ($acao == 'form_requisicao'))
{
                    ...
                    <fieldset>
                        ...
                        <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
                            <tr>
                                ...
                                if (chmodbtn($id_login,"procurar_if","requisicao.php"))
                                {                                
                                ...
                                echo "
                                        ...
                                        <td>".ChmodBtn($id_login,'procurar','requisicao.php')."</td>
                                </form>
                            </tr>
                        </table>
                    </fieldset>
    ...
    if (chmodbtn($id_login,"listar_if","requisicao.php"))
    {
            echo "<table width=98% align=center cellspacing=0 cellpadding=0 border=0>
                    <tr>
                        <td>
            ...
            $sql=db_query("SELECT req_codigo, a.set_nome AS desc_saida, b.set_nome AS desc_consumo, req_nr_nota,
			    ...
			    and   set_entrada in (select set_codigo from usuarios_setores
		...
		if ($row['req_finalizado'] == 'S') $finalizado = "SIM";
			...
			";
		if ( (empty($row['nummov'])) and (empty($row['req_finalizado'])))	{
                   echo " <td width=60 style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9;'>".ChmodBtn($id_login,'editar','requisicao.php?acao=form_edit&req_codigo='.$row[req_codigo])."</td>
                        <td width=66 style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9;'>".ChmodBtn($id_login,'apagar','requisicao.php?acao=del&req_codigo='.$row[req_codigo])."</td> ";
	        }
		else
		    {
		    echo "<td> &nbsp; </td> <td> &nbsp; </td> ";
	  ...
	  <td>
	   <fieldset>
	    <legend>Opções de Cadastro</legend>
	     <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
	      <tr>
	       <td width=79><a href=requisicao.php?id_login=$id_login><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/voltar_on.gif border=0></a></td>
     ...
     echo "<input type=hidden name=req_codigo value=$rowrequisicao[novo_codigo]>
	   ...
	   echo "</select>
	      ...
	      <tr id='tb_setor'>
			...
			/*$sql5 = "select * from setor order by set_nome";
			$exe_sql5 = pg_query($sql5);
			*/
			
	         $query = db_query("select * from setor
				 where set_estoque in ('N','S') 
				 and   set_codigo in (select set_codigo from usuarios_setores
				                      where usr_codigo = $id_login)
                                order by set_nome");
	           while($res_exe_query = pg_fetch_array($query)) {
	                 echo "<option value='$res_exe_query[set_codigo]'>$res_exe_query[set_nome]</option>";
	           }
	   echo "</select>
	        </td>
         </tr>
	     ...
	     <tr>
    		...
    		<td><input type=text readonly name=req_nr_nota class=box size=20 value=$rowrequisicao[novo_codigo]></td>
        </tr>
        <tr>
	 ...
	 <tr>
	  <td>
	   <fieldset>
		...
		<td> <b>REQUISICAO DE PRODUTO DE CONSUMO</b></td>
		 <input type='hidden' name='req_saida' value='S'> 
	      </tr>";
	   ...
	   echo "</select>
		...
		<td colspan=2><input type=text readonly name=req_nr_nota class=box size=20 value='$row[req_nr_nota]'></td>
	      </tr>
            <tr>
    		<td width=40>Requisitante:</td>
		...
		<td width=40>Observacao:</td>
	      ...
	      <tr>
	       ...
	       <td colspan=2><a href=itens_requisicao.php?id_login=$id_login&req_codigo=$req_codigo&action=form_inclui_item><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/veritens_on.jpg border=0></a></td> 
	      </tr>
	     </table>
	   </fieldset>
	  </td>
	...
	if (! $set_entrada) {
		...
		<script type=\"text/javascript\">
			alert(\"Nao e possivel incluir a Requisicao. Escolha o setor solicitante.\")
                        setTimeout(\"location='requisicao.php?id_login=$id_login'\", 0);
		</script>";
		exit();
	       }
/*    $select = "SELECT count(*) from req_dispensado
  ...
  /*  $select = "SELECT COUNT(*) 
    			 FROM requisicao
                WHERE set_entrada = $set_entrada
                  AND   req_codigo NOT IN (SELECT req_codigo 
               							     FROM movimento 
               							    WHERE req_codigo = requisicao.req_codigo)
	              AND   req_data > '2011-03-14'";
 */
	       
	       /* Sem validação 
  $select = "SELECT COUNT(distinct r.req_codigo) as soma
  				     FROM itens_requisicao ir
				     JOIN requisicao r
				       ON r.req_codigo = ir.req_codigo
		            WHERE r.set_entrada = $set_entrada
			          AND r.req_data > '2011-03-13'
	                  AND ir.ireq_consolidado = 'D' 
	                  AND req_tipo !='T'";
  
	debug($select, $PHP_SELF, $id_login);
	
    $exec_select = db_query($select);
    $contdispensado = pg_fetch_array($exec_select);
   //echo $select;exit;
	if ($contdispensado[soma] > 0) {
	       print " 
		<script type=\"text/javascript\">
			alert(\"Ha requisicoes pendentes de confirmacao.\")
                        setTimeout(\"location='requisicao.php?id_login=$id_login'\", 0);
		</script>";
		exit();
	       }
    ...
    $select = "SELECT max(setp_codigo), 
               ...
               WHERE set_codigo = $set_saida
                 AND '$req_data' 
             BETWEEN setp_data_inicial AND setp_data_final
    ...
    if(pg_num_rows($exec_select) > 0 || $set_saida == 99404 )
    {
        ...
        echo "
        <script type=\"text/javascript\">
            setTimeout(\"location='itens_requisicao.php?id_login=$id_login&req_codigo=$req_codigo&action=form_inclui_item'\", 0);
        </script>";
    }
