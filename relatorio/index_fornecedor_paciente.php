...
<script>
	...
	function abreRelatorio(di,df){
		...
		url = "fornecedor_paciente.php?di="+di+"&df="+df+"&for_codigo="+for_codigo+"&tp_rel="+tp_rel;
...
echo $common->menuTab(array("Relatorio de gasto com fornecedor por Paciente"));
echo $common->bodyTab("1");
	echo $form->openForm("$PHP_SELF","POST","");
	
		$sqlUnidade = "SELECT for_codigo,
							  for_nome 
					     FROM fornecedor 
						ORDER BY for_nome";
		
		echo $form->inputSelect("for_codigo",null,"Fornecedor",$sqlUnidade,null,null,null,null,'TODOS');
