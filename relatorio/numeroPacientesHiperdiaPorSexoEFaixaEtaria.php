...
<script>
	...
	function abreRelatorio(di,df){
		var cid_codigo = document.getElementById("cid_codigo").value;
		url = "relHiperdiaPorSexoEFaixaEtaria.php?di="+di+"&df="+df+"&cid_codigo="+cid_codigo;
...
echo $common->menuTab(array("Hiperdia por Sexo e tipo de risco"));
echo $common->bodyTab("1");
	echo $form->openForm("$PHP_SELF","POST","relGeladeira");
		echo $form->hiddenForm("acao","gerar");
		$sqlUnidade = "select * from cidade";
		echo $form->inputSelect("cid_codigo",null,"Cidade",$sqlUnidade);
