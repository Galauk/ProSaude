...
<script>
	...
	function abreRelatorio(di,df){
		var gel_codigo = document.getElementById("gel_codigo").value;
		//var uni_codigo = document.getElementById("uni_codigo").value;
		url = "relTemperatura.php?di="+di+"&df="+df+"&gel_codigo="+gel_codigo;
	...
	echo $form->openForm("$PHP_SELF","POST","relGeladeira");
		...
		$sqlGeladeira = "select * 
						   ...
						   join usuarios_setores us
						     on g.set_codigo=us.set_codigo
					      where us.usr_codigo = $id_login";
		echo $form->inputSelect("gel_codigo",null,"Geladeira",$sqlGeladeira);
