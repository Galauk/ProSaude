...
$titulo="PRODUTOS QUE ATINGIRAM O ESTOQUE MINIMO";    //       NOME DO RELATÓRIO
...
function cabeca($Tit, $dtIni, $dtFin, $SetNo, $GruNo, $tpCab, $zerado) {
        ...
        //---------  Cabeçalho dos Dados  ----------------->
         ...
         if ($tpCab == '1') {
             echo " <tr>\n";
		 	 echo "  <td width= '5%' valign='top' align=right>                         </td>\n";
		 	 echo "  <td width='50%' valign='top' align=left > Produto                 </td>\n";
			 echo "  <td width='20%' valign='top' align=left> Estoque Atual&nbsp;&nbsp;&nbsp;</td>\n";
			 echo "  <td width= '20%' valign='top' > Estoque Minimo                                        </td>\n";
			 echo "  <td width= '10%' valign='top' align=left> Difer&ecirc;nca &nbsp;&nbsp;&nbsp;          </td>\n";
			 echo " </tr>\n";
			 echo " <tr>\n";
...
$sql  = "SELECT pro_nome,
        ...
        prset_minimo,
		round(cast (calcula_estoque(produto.pro_codigo, $set_codigo, '$dt_final') as numeric), 0) - prset_minimo as dif
		FROM produto, produto_setor ";
if ($gru_codigo) {
       $sql .= " WHERE produto.pro_codigo = produto_setor.pro_codigo
				 AND gru_codigo = $gru_codigo
				 AND prset_minimo is not null   ";
} else {
       $sql .= " WHERE produto.pro_codigo = produto_setor.pro_codigo
				 AND prset_minimo is not null   ";
}
$sql .= " AND round(cast (calcula_estoque(produto.pro_codigo, $set_codigo, '$dt_final') as numeric), 0) < prset_minimo
