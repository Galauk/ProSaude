...
function Emite_relatorio() {
    ...
    if (data_ini == "") {
        ...
        parent.open("relatorio/rel_PacientesPorFaixaEtariaPrograma.php?programa="+programa+"&centro_estocador="+centro_estocador+"&data_ini="+data_ini+"&data_fim="+data_fim+"&faixa="+faixa,null,"height=400,width=750,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes");
        return true;
    }
}
...
<body>
<fieldset>
<legend>Paciente por Faixa Et&aacute;ria/Programa</legend>
        ...
        <tr>
            ...
            <td>
                <select name="programa" id="programa" class="box">
                    <option value="-1">---- TODOS ----</option>
                    <?
                    $res_prg = pg_query("SELECT prg_codigo, prg_nome FROM programa_atendimento ORDER BY prg_nome");
                    while( $row_prg = pg_fetch_array($res_prg) )
                    { ?>
                    <option value="<?=$row_prg[0]?>"><?=$row_prg[1]?></option>
                    <?
                    }
                    ?>
                </select>
