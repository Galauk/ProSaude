...
function VerData() {
   ...
   gdtInicial = document.frm_FaltPorPeriodo.dt_inicial.value;
   gdtFinal   = document.frm_FaltPorPeriodo.dt_final.value;
   gUnidade   = document.frm_FaltPorPeriodo.uni_codigo.value;

   if (gdtInicial == '') {
       alert ("Informe Data Inicio");
       document.frm_FaltPorPeriodo.dt_inicial.focus();
       return false;
   }
   if (gdtFinal == '') {
       gdtFinal=document.frm_FaltPorPeriodo.dt_final.value=document.frm_FaltPorPeriodo.dt_inicial.value
   }
   var d1=gdtInicial
   var d2=gdtFinal
   ...
   if (CheckDate(dat2,"FINAL")==1) { 
       document.frm_FaltPorPeriodo.dt_final.focus()
       return false
   }
   if  (dat1 > dat2)    { 
   ...
   window.open('FaltososPorPeriodo.php?dt_inicial=' + gdtInicial +
                                       '&dt_final=' + gdtFinal   +
                                     '&uni_codigo=' + gUnidade
                                     ,null
...
echo " <form name=\"frm_FaltPorPeriodo\" method=\"post\" action=\"$PHP_SELF\">\n";
echo "  <fieldset>";
echo "   <legend>Relat&oacute;rio Faltosos por Periodo</legend> \n";
