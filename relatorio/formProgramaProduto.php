...
<body>
<fieldset>
<legend>Programa Produto Paciente</legend>
<form method="post" action="relatorio/rel_programaProduto.php" target='_blank'>
	<table>
		<tr>
			<td width='30' align=right>Programa: </td>
			<td><select name="prg_codigo" id="centro_estocador" class="box">
                <option value="-1">----Todos----</option>
                <?
				// Lista os produtos
				$sql = "select *from programa_atendimento";
				$query = pg_query($sql) or die(pg_last_error());
						while($row = pg_fetch_array($query)) {
				?>
				<option value="<?=$row[prg_codigo]?>"><?=$row[prg_nome]?></option>
				<?
					}
				?>
            </select></td>
