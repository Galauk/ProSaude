...
{
$sql .="(
	select sum(COALESCE(ite.ite_quantidade, 0::numeric)) AS consumo
	 from movimento as mov
	join itens_movimento as ite
	on ite.mov_codigo = mov.mov_codigo
	join produto as prod 
	on prod.pro_codigo = ite.pro_codigo
	where set_saida = '$set_codigo' 
	AND mov_tipo = 'T'
	AND EXTRACT(YEAR FROM mov.MOV_DATA) = '$ano1'
	AND EXTRACT(MONTH FROM mov.MOV_DATA) = '$mes1'
	AND ite.pro_codigo = CONS.pro_codigo
	group by pro_nome
	order by pro_nome) AS CONSMES1, ";
	}
if( $meses > 2 )
{
$sql .="(
	...
	where set_saida = '$set_codigo' 
	AND mov_tipo = 'T'
	AND EXTRACT(YEAR FROM mov.MOV_DATA) = '$ano2'
	AND EXTRACT(MONTH FROM mov.MOV_DATA) = '".( $mes2 < 10 ? "0" : "" )."$mes2' ";
$sql .= "AND ite.pro_codigo = CONS.pro_codigo
	group by pro_nome
	order by pro_nome) AS CONSMES2, ";
}
if( $meses > 3 )
{
$sql .="(
	...
	where set_saida = '$set_codigo' 
	AND mov_tipo = 'T'
	AND EXTRACT(YEAR FROM mov.MOV_DATA) = '$ano3'
	AND EXTRACT(MONTH FROM mov.MOV_DATA) = '".( $mes3 < 10 ? "0" : "" )."$mes3' ";
$sql .= "AND ite.pro_codigo = CONS.pro_codigo
	group by pro_nome
	order by pro_nome) AS CONSMES3, ";
...
$sql .="(
	...
	where set_saida = '$set_codigo' 
	AND mov_tipo = 'T'
	AND EXTRACT(YEAR FROM mov.MOV_DATA) = '$ano4'
	AND EXTRACT(MONTH FROM mov.MOV_DATA) = '".( $mes4 < 10 ? "0" : "" )."$mes4' ";
$sql .= "AND ite.pro_codigo = CONS.pro_codigo
	group by pro_nome
	order by pro_nome) AS CONSMES4, ";
	}
if( $meses > 5 )
{
$sql .="(
	...
	where set_saida = '$set_codigo' 
	AND mov_tipo = 'T'
	AND EXTRACT(YEAR FROM mov.MOV_DATA) = '$ano5'
	AND EXTRACT(MONTH FROM mov.MOV_DATA) = ".( $mes5 < 10 ? "0" : "" )."$mes5 ";
$sql .= "AND ite.pro_codigo = CONS.pro_codigo
	group by pro_nome
	order by pro_nome) AS CONSMES5, ";
	}
$sql .="(
	select sum(COALESCE(ite.ite_quantidade, 0::numeric)) AS consumo
	 from movimento as mov
	...
	where set_saida = '$set_codigo' 
	AND mov_tipo = 'T'
	AND EXTRACT(YEAR FROM mov.MOV_DATA) = '$ano6'
	AND EXTRACT(MONTH FROM mov.MOV_DATA) = ".( $mes6 < 10 ? "0" : "" )."$mes6";
$sql .= "AND ite.pro_codigo = CONS.pro_codigo
	group by pro_nome
	order by pro_nome) AS CONSMES6
