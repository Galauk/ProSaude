...
<script>
	...
	function abreRelatorio(di,df){
		...
		url = "RelatorioProcColetados.php?di="+di+"&df="+df+"&uni_codigo="+uni_codigo+"&tp_rel="+tp_rel;
...
echo $common->menuTab(array("Procedimentos Coletados por Período e Unidade"));
echo $common->bodyTab("1");
	echo $form->openForm("$PHP_SELF","POST","");
	
		$sqlUnidade = "SELECT conv_codigo,med_nome 
						 FROM convenio c
						 JOIN medico m
						   ON m.med_codigo = c.med_codigo";
		
		echo $form->inputSelect("conv_codigo",null,"Convênio",$sqlUnidade,null,null,null,null,"TODAS");
