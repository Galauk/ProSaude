...
<script>
	...
	function abreRelatorio(di,df){
		...
		url = "rel_atendimentoMedicoMensal.php?di="+di+"&df="+df+"&usr_codigo="+usr_codigo+"&uni_codigo="+uni_codigo+"&tp_rel="+tp_rel;
...
echo $common->menuTab(array("Quantidade por Atendimentos Mensal por Medico"));
echo $common->bodyTab("1");
	echo $form->openForm("$PHP_SELF","POST","");
	
		$sqlMedico = "SELECT usr_codigo,usr_nome FROM usuarios WHERE usr_tipo_medico IN ('M','D') and usr_ativo = 'S' ORDER BY usr_nome";
