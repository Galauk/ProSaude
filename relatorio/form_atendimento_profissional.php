...
<script>
	...
	function abreRelatorio(di,df){
		...
		url = "atendimentoProfissional.php?di="+di+"&df="+df+"&usr_codigo="+usr_codigo+"&tp_rel="+tp_rel+"&ate_tipo="+ate_tipo;
...
echo $common->menuTab(array("Quantidade de Atendimentos por Profissional"));
echo $common->bodyTab("1");
	echo $form->openForm("$PHP_SELF","POST","");
	
		$sqlUsu = "SELECT usr_codigo,usr_nome FROM usuarios ORDER BY usr_nome";
		...
		echo $form->inputSelect("usr_codigo",null,"Profissional",$sqlUsu,null,null,null,null,'TODOS');
		...
		echo $form->inputCheckboxRadio("ate_tipo",null,"Tipos de Atendimento",null,$arrayTipoAtendimento,"radio");
