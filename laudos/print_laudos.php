...
$resSql = pg_fetch_array($querySql);
						...
						if($numLinhasSubExame > 0){
							...
							$sqlTipoExame = "SELECT * FROM tipodeexame where proc_codigo = $val";
							...
							$sqlTipoDoExameItem = "SELECT distinct i.ite_codigo,
														 i.ite_itemdoexame,
							...
							while($regTipoDoExameItem = pg_fetch_array($queryTipoDoExameItem)){
								...
								echo"
													...
													$sqlItensDeAnalise = "SELECT i.ite_codigo,
																				 i.ite_itemdoexame,
																				 v.vlr_valordereferencia || ' ' || i.ite_tipo_medida as vlr_valordereferencia
																			FROM itensanalise i
																			LEFT JOIN valoresdereferencia v
																			  ON i.ite_codigo = v.ite_codigo
																		   WHERE i.ite_codigo = $regTipoDoExameItem[ite_codigo]
																			 AND (vlr_faixa_etaria_inicio <= ".intval($idadeMeses)."
																				 OR vlr_faixa_etaria_inicio IS NULL)
																			 AND (vlr_faixa_etaria_fim > ".intval($idadeMeses)."
																				  OR vlr_faixa_etaria_fim IS NULL)
																			 AND (vlr_sexo = '$regMesesUsuario[usu_sexo]' OR vlr_sexo is null)
																			ORDER BY i.ite_codigo";
														//die($sqlItensDeAnalise);
													$queryValoresDeReferencia = pg_query($sqlItensDeAnalise);
													//die(pg_num_rows($queryValoresDeReferencia)."aa");
													$regValoresDeReferencia = pg_fetch_array($queryValoresDeReferencia);
													echo "<font size=1>".$regValoresDeReferencia[vlr_valordereferencia].$regItens[ite_tipo_medida]."<br/>"; 
