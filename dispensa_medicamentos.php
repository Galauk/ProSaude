...
function buscarProdutos( txt )
{
	...
	if(tip == "salvar")
	{
		endereco = 'buscarProdutosDispensados.php?mov_codigo='+mov_codigo;
		ajax_tudo(endereco,atualizarGrid);
	} else if(tip == "editar") {		
...
function trataResultado( txt )
{
	if(txt == "yes")
	{
		...
		flag_medico = document.getElementById('flag_medico').checked;
		...
		{
			document.getElementById('mov_codigo').value = '';
			...
			ajax_salvar.send("mov_data="+data+"&mov_tipo=S&usu_codigo="+codigo+"&set_saida="+set_codigo+"&usr_codigo="+id_login+"&mov_saida=D&ate_codigo="+ate_codigo+"&med_codigo="+med_codigo+"&flag_medico="+flag_medico);
...
{
	exec_ajax('s.php?busca='+valor, 'produto');
}
...
function gravar(tipo)
{
	...
	{	
		...
		window.open('escolherLoteValidade.php?pro_codigo='+pro_codigo+'&mov_codigo='+mov_codigo+'&quantidade='+quantidade+'&pro_nome='+pro_nome+'&mov_codigo='+mov_codigo+'&pro_codigo='+pro_codigo+'&ite_status=A&ite_quantidade='+quantidade+'&ite_consolidado='+ite_consolidado+'&ite_qtde_solicitada='+quantidade+'&ite_posologia='+posologia+'&ite_detalhes_tratamento='+detalhes+'&ite_observacoes='+observacoes+'&quantidade_dia='+quantidade_dia+"&set_codigo="+set_codigo,null,"height=400,width=750,status=yes,toolbar=no,menubar=no,resizable=yes, location=no,scrollbars=yes");
...
/*function teste(){
	...
	observacoes = document.getElementById('observacoes').value;
	quantidade_dia = document.getElementById('quantidade_dia').value;
	url = "salvarProdutosDispensados.php?mov_codigo="+mov_codigo+"&pro_codigo="+pro_codigo+"&ite_status=A&ite_quantidade="+quantidade+"&ite_consolidado="+ite_consolidado+"&ite_qtde_solicitada="+quantidade+"&ite_posologia="+posologia+"&ite_detalhes_tratamento="+detalhes+"&ite_observacoes="+observacoes+"&quantidade_dia="+quantidade_dia;
	ajax_tudo(url, novoTeste)
}

function novoTeste(txt){
	if (txt == "Inserido"){
		ajax_tudo('buscarProdutosDispensados.php?mov_codigo='+mov_codigo, atualizarGrid);
	} else if(txt == "Alterado") {
		ajax_tudo('buscarProdutosDispensados2.php?mov_codigo='+mov_codigo, atualizarGrid);
}*/

/*
 * Comentado pois não está sendo utilizado mas poderá ser no futuro.
 */
 /*function buscarCota()
{
...
function respostaCota(resp,contagem)
{
	//mov_codigo = document.getElementById("mov_codsdsdsigo").value;
	...
	endereco = "salvarProdutosDispensados.php?mov_codigo="+mov_codigo+"&contagem="+contagem+"&resp="+resp+"&pro_codigo="+pro_codigo+"&ite_status=A&ite_quantidade="+quantidade+"&ite_consolidado="+ite_consolidado+"&ite_qtde_solicitada="+quantidade+"&ite_posologia="+posologia+"&ite_detalhes_tratamento="+detalhes+"&ite_observacoes="+observacoes+"&quantidade_dia="+quantidade_dia;
	//alert(endereco)
	ajax_tudo(endereco,buscarProdutos);

	//alert("mov_codigo="+mov_codigo+"&pro_codigo="+pro_codigo+"&ite_status=A&ite_quantidade="+quantidade+"&ite_consolidado="+ite_consolidado+"&ite_qtde_solicitada="+quantidade+"&ite_posologia="+posologia+"&ite_detalhes_tratamento="+detalhes+"&ite_observacoes="+observacoes+"&quantidade_dia="+quantidade_dia+"&lote="+lote+"&validade="+validade);
/***************************************	
	novo_ajax = ajaxInit();
   // alert('chamo salvar produtos');
	...
	novo_ajax.send("mov_codigo="+mov_codigo+"&pro_codigo="+pro_codigo+"&ite_status=A&ite_quantidade="+quantidade+"&ite_consolidado="+ite_consolidado+"&ite_qtde_solicitada="+quantidade+"&ite_posologia="+posologia+"&ite_detalhes_tratamento="+detalhes+"&ite_observacoes="+observacoes+"&quantidade_dia="+quantidade_dia+"&lote="+lote+"&validade="+validade);
//	alert(novo_ajax.responseText);	
//document.getElementById("gridProdutos").innerHTML = novo_ajax.responseText;	
//	alert('chamo salvar produtos');
	alert("Produto Cadastrado Com Sucesso");
//	alert(novo_ajax.responseText);
	//alert(novo_ajax.readyState);
//	alert(novo_ajax.responseText);
	//alert('buscarProdutosDispensados.php?mov_codigo='+mov_codigo);
...
function buscarProdutosDispensados(txt){
	alert(txt);
	document.getElementById("gridProdutos").innerHTML = txt;
}
...
{
  window.open("../.php?acao=form_add&type=c&id_login=$id_login&controle=1",null,"height=460,width=800,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes");
)};*/
...
<script language="JavaScript" type="text/javascript" src="exame/procedimento.js"></script>
                      ...
                      echo "<tr>";
                              echo "<td>";
                                      echo "<fieldset>";
                                              ...
                                              echo "<table class='lista' width=100% align=center cellspacing=2 cellpadding=4 border=0>";
                                                      echo "<tr bgcolor=F9f9f9>";
                                                              echo "<th width=100>Data</th>";
                                                              echo "<th width=160>Centro Estoc. Saida</th>";
                                                              echo "<th width=350>Paciente</th>";
                                                              echo "<th width=30>Num.Movimento</th>";
                                                              echo "<th width=30colspan=2>Imprimir</th>";
                                                              ...
                                                              while($row=pg_fetch_array($sql))
                                                              {
                                                                      ...
                                                                      echo "<tr>";
                                                                              ...
                                                                              echo "<td width=40 align=center>".
                                                                              	$common->commonButton("imprimir", null, "print.png", "onclick=\"imprimir($id_login, $row[mov_codigo]);\"")
                                                                              .
                                                                              "
                                                                              			</td>";
                                                                              /*
                                                                              			<span style='cursor:pointer;' onclick=\"imprimir($id_login, $row[mov_codigo]);\"/>
                                                                              				<img src='".$_SESSION[linkroot].$_SESSION[comum]."imgs/imprimir_on.jpg' align='top' border=0>
                                                                              			</span>
                                                                               * */
...
//vsql($sqlv,"1");          
	...
	{
		if($del = "del"){
		$del = "delete from itens_movimento where mov_codigo =$mov_codigo";
		$del2 = "delete from movimento where mov_codigo =$mov_codigo";
		$exec = pg_query($del);
		$exec2 = pg_query($del1);
		}
//		$select = "select set_saida, to_char(mov_data, 'dd/mm/yyyy') as data, usu_codigo, mov_num_receita from movimento where mov_codigo = $mov_codigo";
		$select = "select set_saida, to_char(mov_data, 'dd/mm/yyyy') as data, usu_codigo from movimento where mov_codigo = $mov_codigo";
				...
				echo "<table width=98% cellspacing=0 cellpadding=0 border=0>";
					echo "<tr>";
						...
						echo "<td>";
							...
							echo "<input type=\"text\" size=\"15\" name=\"data\" id=\"data\" value=\"$date\" readonly class=\"box\" onKeyPress=\"return Ajusta_Data(this, event)\" maxlength=\"10\" readonly>";
						echo "</td>";
			...
			echo "<fieldset>";
				...
				echo "<table width=98% cellspacing=0 cellpadding=0 border=0>";
					echo "<tr>";
						...
						echo "</td>";
						echo "<td width=\"220px\">";
							echo "<input type=hidden name=ate_codigo id=ate_codigo>";
							echo "<select name=\"medico\" id=\"medico\" class=\"boxl\" onchange=\"buscarConsultas(this)\">";
								echo "<option value=\"\">--> ESCOLHA <--</option>";
								$select = "select * from medico where  prestador_servicos = 'N'  order by med_nome";
								$exec_select = db_query($select);
			...
			echo "<div id=\"resposta\">";
				echo "<fieldset>";
					...
					echo "<table width=98% cellspacing=0 cellpadding=0 border=0>";
						echo "<tr>";
							echo "<td width=520>";
								echo "<input type=hidden name=pro_codigo id=pro_codigo>";
								echo "<input type=hidden name=ite_consolidado id=ite_consolidado>";
				...
				echo "<table width=98% cellspacing=0 cellpadding=0 border=0>";
					echo "<tr>";
						...
						echo "<td>";
							...
							echo "<input type=\"text\" size=\"15\" name=\"data\" id=\"data\" value=\"$date\" readonly class=\"box\" onKeyPress=\"return Ajusta_Data(this, event)\" maxlength=\"10\">";
						echo "</td>";
                            ...
                            echo "<table width=98% cellspacing=0 cellpadding=1 border=0>";
                                echo "<tr>";
                                    ...
                                    echo "<td width=\"40px\">";
                                        echo "M&eacute;dico";
                                    echo "</td>";
#onchange=\"buscarConsultas(this)
                                    echo "<td width=\"220px\">";
                                        echo "<input type=hidden name=ate_codigo id=ate_codigo value=\"$_GET[ate_codigo]\">";
                                        echo "<select name=\"medico\" id=\"medico\" class=\"boxl\">";
                                            echo "<option value=\"\">--> ESCOLHA <--</option>";
                                            $select = "select * from usuarios where usr_tipo_medico = 'M' order by usr_nome";
                                            $exec_select = db_query($select);
                                            while($linha = pg_fetch_array($exec_select))
                                            {
                                                if($row['usr_codigo'] == $linha['usr_codigo'])
                                                {
                                                    echo "<option value=\"$linha[usr_codigo]\" selected>$linha[usr_nome]</option>";
                                                } else {
                                                    echo "<option value=\"$linha[usr_codigo]\">$linha[usr_nome]</option>";
                                                }
                                            }
                                        echo "</select>";
                                    echo "</td>";
...
$exec_sql22 = pg_query($sqlSetor2);
 ...
 /*} else {
							...
							echo "<select name=set_saida id=set_saida class=box>";
                                $exec_sql = db_query($sql);
								while($setor=pg_fetch_array($exec_sql22))
								{
									echo "<option value='$setor[set_codigo]'>$setor[set_nome]</option>";
								}
							echo "</select>";
...
/*echo "<fieldset  style='background-color:#FDD1D1'>";
                                ...
                                echo "<table width=100% cellspacing=0 cellpadding=1 border=0>";
                                        echo "<tr>";
                                                ...
                                                echo "<a href='#' onchange=\"buscar_nome(\$F('pac_nome'), 'buscar_nome');return false;link_f7()\"><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/localizar.jpg id=localizar align=absmiddle border=0></a>";
			...
			echo "<fieldset>";
				...
				echo "<table width=100% cellspacing=0 cellpadding=1 border=0>";
					echo "<tr>";
						...
						echo "<a href='#' onclick=\"buscar_nome(\$F('pac_nome'), 'buscar_nome','F');return false;\"><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/localizar.jpg id=localizar align=absmiddle border=0></a>";
			...
			echo "<fieldset>";
				...
				echo "<table width=98% cellspacing=0 cellpadding=1 border=0>";
					echo "<tr>";
						...
						echo "<td>
							   <div id='oc'>";
									$sle = "select usr_codigo,usr_nome from usuarios where usr_tipo_medico = 'M'";
									$checked = "";
								$select = pg_query($sle);
								echo"
			       					<select id=medico name=medico class=box value='$sel[med_codigo]' style='width:210px;'>
							       		 <option value='$sel[med_codigo]'>..:: Selecione o Medico ::..</option>";
		
								while($med=pg_fetch_array($select)) {
									echo ($med[0]==$cx[med_codigo])?"<option value='$med[0]' selected>$med[1]</option>":"<option value='$med[0]'>$med[1]</option>";
								}
								echo "
		        					 </select>
										&nbsp;<input type ='checkbox' onclick='xi()' name='cad_medico_externo' id='flag_medico' value='E' $checked><b>Somente Externos</b>
		       						 </div>";
						echo "</td>";
						echo "<input type=hidden name=ate_codigo id=ate_codigo readonly>";
							...
							echo "</select>";
							 echo "<select name=\"medico\" id=\"medico\" class=\"boxl\">";
                                                                echo "<option value=\"\">--> ESCOLHA <--</option>";
                                                                $selectExterno = "select * from medico where prestador_servico = 'N'";
                                                                $selectQuery = pg_query($selectExterno);
                                                                while($linha2 = pg_fetch_array($selectQuery))
                                                                {
                                                                        echo "<option value=\"$linha2[med_codigo]\">$linha2[med_nome]</option>";
                                                                }
                                                        echo "</select>";
			...
			echo "<div id=\"resposta\" style=\"display:none\">";
				echo "<fieldset>";
					...
					echo "<table width=98% cellspacing=0 cellpadding=0 border=0>";
						echo "<tr>";
							echo "<td width=520>";
								echo "<input type=hidden name=pro_codigo id=pro_codigo>";
								echo "<input type=hidden name=ite_consolidado id=ite_consolidado>";
								echo "<input type=text name=pro_nome id=pro_nome class=boxl size=100>";
							echo "</td>";
							echo "<td width=25>";
							 echo "<a href='#' onclick=\"buscar_produto(\$F('pro_nome'));return false;\"><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/localizar.jpg id=localizar align=absmiddle border=0></a>";
							 	echo divBuscaProduto();
								/*echo "<input type=image src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/localizar.jpg onclick=\"buscaCad2('buscarProdutos.php','produto',document.getElementById('set_saida').value)\">";*/
							echo "</td>";
							echo "<td width=40>";
								echo "Quantidade:";
							echo "</td>";
							echo "<td>";
								echo "<input type=text name=quantidade id=quantidade class=box size=5 onChange=buscarEstoque(this.value) onkeypres=\"return Bloqueia_Caracteres(event);\">";
							echo "</td>";
							echo "<td width=100>";
								echo "Quantidade Dia:";
							echo "</td>";
							echo "<td>";
								echo "<input type=text name=quantidade_dia id=quantidade_dia class=box size=5 onChange=dividir() onkeypres=\"return Bloqueia_Caracteres(event);\">";
							echo "</td>";
							echo "<td width=80>";
								echo "Total Dias:";
							echo "</td>";
					...
					echo "<table width=98% cellspacing=0 cellpadding=0 border=0>";
						...
						echo "<tr>";
							echo "<td>";
								echo "<img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/add_on.gif onclick=gravar('salvar')>";
							echo "</td>";
							echo "<td>";
								echo "<img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/finalizar_on.jpg onclick=\"location.href='dispensa_medicamentos.php?id_login=$id_login&acao=listar'\">";
							echo "</td>";
						echo "</tr>";
					echo "</table>";
				echo "</fieldset>";
