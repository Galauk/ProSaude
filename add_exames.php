...
if($act=="addproc") {
   $ins = pg_query("insert into procedimento_atendimento (ate_codigo,proc_codigo) values ('$ate_codigo','$exames')");
        echo "<SCRIPT LANGUAGE=\"JavaScript\">
                  setTimeout(\"location='add_exames.php?id_login=$id_login&ate_codigo=$ate_codigo'\", 0);
              </SCRIPT>";
}
if($act=="") {
$sel = pg_query("select *from procedimento_atendimento where ate_codigo='$ate_codigo'");
while($row=pg_fetch_array($sel)) {
$proc = pg_fetch_array(pg_query("select *from procedimento where proc_codigo = '$row[proc_codigo]'"));
if(($proc[proc_exame]=='S')) {
