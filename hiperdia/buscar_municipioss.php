<?php
	...
	if(strlen($palavra) > 0)
	{		
		if($acao == "buscar_municipio")
		{
			$where = " cid_nome like upper('".trim($palavra)."%') ";
		}

		$sql = "select cid_codigo,cid_nome,cid_codigo_ibge,uf_sigla from cidade where $where";
	...
	while($row = pg_fetch_array($exec))
	{
		...
		echo "
			<tr id='tr$i' onmouseover=\"trocar_cor(this.id, 'fone$i');\" onmouseout=\"retirar_cor(this.id, 'fone$i')\"
				onclick=\"passar_cidade($row[cid_codigo_ibge], '$row[cid_nome]', '$row[uf_sigla]')\" style='cursor:pointer;'>
				<td align='left'>
					$row[cid_codigo_ibge]
				</td>
				<td align='left'>
