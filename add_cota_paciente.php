...
echo monta_janela("editar","EDITAR DISPENSA��O");
...
// Gambiarra pra atualizar o grid apos a edição
var timeoutinterval = "";
...
function respostaCota(txt)
{
	...
	// a = produto não faz parte de nenhum programa -> então pode pegar
	// b = caso o produto faça parte de algum programa verifica se o paciente faz parte de algum programa com aquele medicamento, caso o paciente não faça é retornado o valor b
	// txt != "a" && txt != "b" && txt != "c" = é retornado o valor da cota do paciente e o valor da ultima vez que ele pegou, levando em consideração qual é o tipo de periodo da cota do paciente
...
<?php
	...
	//-> Listando
            if (chmodbtn($id_login,"listar_if","dispensa_medicamentos.php")) // faz funcionar a permiss�o "listagem" da dispensa��o de medicamento
            {        
                echo "<table width=98% align=center cellspacing=0 cellpadding=0 border=0>";
                      echo "<tr>";
                              echo "<td>";
...
//vsql($sqlv,"1");          
	...
	else if($acao == "detalhe")
	{
		...
		echo "<body onKeyPress=\"verificarCota(event)\">";
			echo "<fieldset>";
				echo "<legend>Dispensação de Medicamentos</legend>";
	...
	else
	{
		echo "<body onKeyPress=\"verificarCota(event)\">";
			...
			echo "<fieldset>";
				...
				echo "<table width=100% cellspacing=0 cellpadding=1 border=0>";
					echo "<tr>";
						echo "<td width=110>Prontuário</td>";
						echo "<td width=40>";
