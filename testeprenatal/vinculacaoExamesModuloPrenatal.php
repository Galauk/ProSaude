...
echo $common->menuTab(array("Exames Prenatal"));
	...
	if($acao == ""){
		echo $form->openForm("$PHP_SLEF","POST","vincula");
			...
			echo $form->inputSelect("proc_codigo","$proc_codigo","Exame","$sql",null,null,null,30,null,30);
	...
	if($acao == "vincular"){
		...
		if($query = pg_query($sql)){
			...
			echo $common->modalMsg("ERRO","Erro ao salvar!","vinculacaoExamesModuloPrenatal.php");
		}
	}
	if($acao == "apagar"){
		if($acao2 == ""){
