...
echo "var linkroot = '{$_SESSION[linkroot]}';";
echo "var comum = '{$_SESSION[comum]}';";
?>
/**
...
function link_f7( )
{
	window.open( linkroot+comum+'list_pacientes.php?id_login='+id_login+'&controle=1',
                                 null,
...
function link_f8( )
{
	...
	window.open( 'paciente.php?acao=form_add&id_login='+id_login+'&controle=1',
				 null,
				 'height=595,width=800,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes');
...
*/
function pacientes( codigo, nome, nascimento, mae, cidade,responsavel, area, microarea,rua,bairro,numero )
{
	...
	$('pac_res').value 	        = responsavel;
	$('area').value 	        = area;
	$('microarea').value 	    = microarea;
	$('rua_nome').value         = rua;
	$('rua_bairro').value         = bairro;
	$('dom_numero').value         = numero;
...
function at_iframe_esq( msg )
{
	    ...
	    age_item == 0 || ! pac_codigo || ! pac_nome )
	{
		...
		$('btn_enviar').src = '<?= $_SESSION[linkroot].$_SESSION[comum];?>imgs/enviar_off.jpg';

		return false;
	}
	
	$('btn_enviar').src = '<?= $_SESSION[linkroot].$_SESSION[comum];?>imgs/enviar_on.jpg';
	...
	var url = 'agendamento_fazer.php'+
				...
				'&age_tipo='+ age_tipo +
				'&age_item='+ age_item +
				'&age_hora='+ hora; //+'&proc_codigo='+proc_codigo;
