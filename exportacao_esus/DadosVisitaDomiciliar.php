...
class ThriftVisitaDomiciliar {
	...
	public function executeMain($eeh_codigo){
		...
		$cab = "Total Visita Domiciliar: ".$numAtend."\n";
		log_exportacao($cab);
		$controle=0;		
		if ($numAtend > 0) {
			...
			foreach($dadosVisitaDomiciliar as $row) {
			   $controle++;
                $info = "Visita Domicilar: ".$controle." Serial:" .$uuid."\n";
                log_exportacao($info);
				//GERA��O DO ARQUIVO
	...
	public function fichaVisitaDomiciliarChildThrift($row){
		$array = array(
			...
			"cnsCidadao" => ($this->validaCnsBanco(trim($row['esv_usu_cns']))==true ? trim($row['esv_usu_cns']) : NULL),
			"sexo" => $row['esv_usu_sexo'],
			"statusVisitaCompartilhadaOutroProfissional" => $row['visita_compartilhada_por_profissional'],
			"motivosVisita" => $this->motivoVisita($row['co_cds_visita_domiciliar']),
			"desfecho" => $row['esv_desfecho'],
			"microarea" => $row['usu_microarea'],
			"stForaArea" => $row['st_fora_area'],
			"tipoDeImovel" => $row['visita_tipo_domicilio'],
	...
	private function montaArrayEnvioDto($dadosVisitaDomiciliarSerializado,$uuid,$row){
		...
		$array_envio_dto = array(
			...
			"cnesDadoSerializado" =>  $row['esv_cnes'] ? $row['esv_cnes'] : '',
