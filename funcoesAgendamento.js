...
function Verifica_Data(data, obrigatorio){
 ...
 function Bloqueia_Caracteres2(evnt){
 //Fun��o permite digita��o de n�meros
 	if (clientNavigator == "IE"){
 		if (evnt.keyCode < 48 || evnt.keyCode > 57){
 			return false
		...
		if(evnt.charCode != 47)
		{
			if ((evnt.charCode < 46 || evnt.charCode > 57) && evnt.keyCode == 0){
				return false;
			}
		} else {
			return false;
...
function validaTIT(rcpf1)
{
	...
	*/
	...
	/*
	 Funções para busca de medicamentos
	*/
	function buscar_medicamento(valor, acao, setor)
	{
		url = "buscar_medicamentos.php?palavra="+valor+"&acao="+acao+"&setor="+setor;
		ajax_tudo(url, popular_medicamento);
		$('lista_medicamentos').style.display = '';
		$('table_medicamentos').innerHTML = '';
		$("lista_carregando_medicamentos").style.display = '';
	}
	
	function popular_medicamento(txt)
	{
		try {
			t = $('table_medicamentos');
			$("lista_carregando_medicamentos").style.display = 'none';
			t.innerHTML = txt;
		} catch(e) {
			alert(e);
		}
	...
	function passar_medicamento(pro_codigo, pro_nome)
	{
		$('pro_codigo').value = '';
		$('pro_nome').value = '';
		$('quantidade').value = '';
		$('ite_consolidado').value = '';
		$('posologia').value = '';
		$('detalhes').value = '';
		$('observacoes').value = '';
		$('quantidade_dia').value = '';
		$('total_dias').value = '';
		$('qtde_calc').value = '';
		$('pro_codigo').value = pro_codigo;
		$('pro_nome').value = pro_nome;
		$('lista_medicamentos').style.display = 'none';
	}
