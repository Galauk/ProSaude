<?php
/**
 * alterações dia 25/05/2007: adionado o link_extra que serve para se o usuário vir de farmacia.php ele quando for voltar para farmacia.php
*/
session_start();
?>
...
//------------------------------------------------------------------>
 if ($action == 'form_inclui_item') {
  ...
  echo "<table width=98% align=center cellspacing=0 cellpadding=0 border=0>
	 <tr>
	  <td>
	   <fieldset>
	    <legend>Dados da Requisicao</legend>
 ...
 echo "
		...
		<td><input type=text name=req_data class=box size=20 value='$row[req_data]'></td>";
	      ...
	      <tr>";
				echo "<td><a href=alterarequisicao.php?id_login=$id_login&req_codigo=$req_codigo&link_extra=$link_extra><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/voltar_on.gif border=0></a></td>";
	      echo "</tr>
	     </table>
	   </fieldset>
	  </td>
...
/*
MARCO - 23/07/2008
ROTINA DE INCLUSAO DE PRODUTOS EM UMA REQUISICAO - TIRADA PARA NAO PERMITIR A INCLUSAO
NESTE PROGRAMA QUE E SO PARA ALTERACAO.
  echo "<table width=98% align=center cellspacing=0 cellpadding=0 border=0>
	 <tr>
	  <td>
	   <fieldset>
		...
		$sql = "select * from produto
				where pro_codigo in (select pro_codigo
						     from produto_setor
						     where set_codigo = $rowcentestoc[set_codigo])
			        and pro_tipo <> 'M'
				order by pro_nome";
...
//              <td><b><div style='width:20px; text-align:left' id='qtd_estoque'> </div> </b></td>
	   echo "</select>
	       ...
	       <td width=79>";
			if(pg_num_rows($exec_sel) > 0)
			{
				echo "<a href=alterarequisicao.php?id_login=$id_login&requis=$req_codigo&final=1><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/finalizar_on.jpg border=0></a>";
			} else {
				echo "&nbsp;";
			}
		   echo "</td>
           <td width=79>";
				echo "<a href=alterarequisicao.php?id_login=$id_login&req_codigo=$req_codigo&link_extra=$link_extra><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/voltar_on.gif border=0></a>";
			echo "</td>
	      </tr></form>
	     </table>
	   </fieldset>
...
//-> Listando
  echo "<table width=98% align=center cellspacing=0 cellpadding=0 border=0>
	 <tr>
	  <td>
	   <fieldset>
		...
		<td width=40 style='border-bottom:1px solid;border-right:1px solid;border-color:c9c9c9;'>Situacao</td>
   ...
   $sql=pg_query("select ireq_codigo, itens_requisicao.pro_codigo, pro_nome,  ireq_quantidade, ireq_vlrunit, ireq_vlrdesc,
	          ...
	          case when ireq_consolidado = 'R' then 'REQUISITADO'
		       when ireq_consolidado = 'D' then 'DISPENSADO '
		       when ireq_consolidado = 'A' then 'ATENDIDO   '
		       when ireq_consolidado = 'C' then 'CANCELADO  '
		  end as desc_status, ireq_consolidado
                  from itens_requisicao, produto
                  where itens_requisicao.pro_codigo = produto.pro_codigo
                  and   (select req_tipo from requisicao where req_codigo = itens_requisicao.req_codigo) = 'S'
                  and   req_codigo = $req_codigo
		  and   ireq_consolidado <> 'C'
   ...
   <td align=center style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9;'>$row[desc_status]</td>
	   ...
	   if (($row['ireq_consolidado'] <> 'C') and ($row['ireq_consolidade'] <> 'A'))  {
...
if ($acao == 'form_edit') {
 ...
 echo "
         ...
         <tr>
            <td width=70>Setor</td>
            <td width=70 ><input type=text readonly name=setor_nome size=40 value='$rowsetor[set_nome]'></td>
            <td width=70>Tipo de Requisicao</td>
            <td width=70 ><input type=text readonly name=tiposaida size=40 value='$row[tiposaida]'></td>
         </tr>
...
//-> Botoes
  ...
  echo "<table width=98% align=center cellspacing=0 cellpadding=0 border=0>
	 <tr>
	  <td>
	   <fieldset>
	    <legend>Alteracao do Item da Requisicao</legend>
	     <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
		...
		<input type=hidden name=dataestoque value=$rowdata>
	     ...
	     <tr>
	       <td width=95><input type=image src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/editar_on.jpg></td>
               <td width=79><a href=itens_alterarequisicao.php?id_login=$id_login&req_codigo=$req_codigo&action=form_incluir_item><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/voltar_on.gif border=0></a></td>
	      </tr></form>
	     </table>
	   </fieldset>
	  </td>
...
//-> EDIT <--------------------------------------------------------->
 ...
 if($action=="edit") {
    ...
    $sql = pg_query("update itens_requisicao set " .
            ...
            ($ireq_vlrunit ? "ireq_vlrunit='$ireq_vlrunit'" : "ireq_vlrunit=null") . ", " .
            ($req_codigo ? "req_codigo='$req_codigo'" : "req_codigo=null") . ", " .
...
//-> DEL <---------------------------------------------------------->
 ...
 if($action=="form_exclui_item") {
  $sql = pg_query("update itens_requisicao 
                   set ireq_consolidado = 'C', ireq_quantidade = null
		   where ireq_codigo='$ireq_codigo'");
  $query = "update itens_requisicao 
                   ireq_consolidado = 'C', ireq_quantidade = null
		   where ireq_codigo='$ireq_codigo'";
       echo "<SCRIPT LANGUAGE=\"JavaScript\">
                setTimeout(\"location='alteraitens_requisicao.php?id_login=$id_login&req_codigo=$req_codigo&action=form_inclui_item'\", 0);
          </SCRIPT>";
