...
reglog($id_login,"Entrando em ALTERACAO REQUISICAO DE TRANSFERENCIA DE MEDICAMENTOS");
//------------------------------------------------------------------>

echo "<fieldset><legend>ALTERACAO DE REQUISICAO DE TRANSFERENCIA DE MEDICAMENTOS </legend>";
...
if (empty($acao) || ($acao == 'form_requisicao'))
{
                    ...
                    <fieldset>
                        ...
                        <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
                            <tr>
                                ...
                                <td width=95>".ChmodBtn($id_login,'adicionar','alteratransferencia.php?acao=form_add')."</td>";
                                if (chmodbtn($id_login,"procurar_if","alteratransferencia.php"))
                                {                                
                                ...
                                echo "
                                        ...
                                        <td>".ChmodBtn($id_login,'procurar','alteratransferencia.php')."</td>
                                </form>
                            </tr>
                        </table>
                    </fieldset>
    ...
    if (chmodbtn($id_login,"listar_if","alteratransferencia.php"))
    {
            echo "<table width=98% align=center cellspacing=0 cellpadding=0 border=0>
                    <tr>
                        <td>
		...
		if ( (empty($row['nummov'])) ) 	{
                   echo " <td width=60 style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9;'>".ChmodBtn($id_login,'editar','alteratransferencia.php?acao=form_edit&req_codigo='.$row[req_codigo])."</td>
                        <td width=66 style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9;'>".ChmodBtn($id_login,'apagar','alteratransferencia.php?acao=del&req_codigo='.$row[req_codigo])."</td> ";
	        }
		else
		    {
		    echo "<td> &nbsp; </td> <td> &nbsp; </td> ";
...
if( $acao=="busca" ) {
    //
    //-> Verificando Busca
    reglog($id_login,"Buscando em ALTERACAO ALTERACAO REQUISICAO DE TRANSFERENCIA DE MEDICAMENTOS: $palavra_chave ");
   ...
   $sqlv="SELECT req_codigo, a.set_nome AS desc_saida, b.set_nome AS desc_consumo,
		  ...
		  (select mov_codigo from movimento where req_codigo = requisicao.req_codigo) as nummov, req_finalizado
                  ...
                  OR    upper(case when req_saida = 'T' then 'Requisicao de Consumo'
...
if($acao=="form_add")
{
    reglog($id_login,"Formulario de ADICAO ALTERACAO REQUISICAO DE TRANSFERENCIA DE MEDICAMENTOS");
	  ...
	  <td>
	   <fieldset>
	    <legend>Opções de Cadastro</legend>
	     <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
	      <tr>
	       <td width=79><a href=alteratransferencia.php?id_login=$id_login><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/voltar_on.gif border=0></a></td>
...
//------------------------------------------------------------------>
 ...
 if($acao=="form_edit") {
	 reglog($id_login,"Formulario de EDICAO ALTERACAO REQUISICAO DE TRANSFERENCIA DE MEDICAMENTOS");
	 ...
	 <tr>
	  <td>
	   <fieldset>
		...
		<td> <b>ALTERACAO REQUISICAO DE TRANSFERENCIA DE MEDICAMENTOS </b></td>
		 <input type='hidden' name='req_saida' value='T'> 
	      </tr>";
	   ...
	   echo "</select>
		...
		<td width=140>Observacao:</td>
	      ...
	      <tr>
	       <td width=79 ><a href=itens_alteratransferencia.php?id_login=$id_login&req_codigo=$req_codigo&action=form_inclui_item><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/veritens_on.jpg border=0></a></td> 
	      </tr>
	     </table>
	   </fieldset>
	  </td>
...
if($acao=="add")
{
	if( $req_saida == 'A' || $req_saida == 'D' || $req_saida == 'P' )
        ...
        reglog($id_login,"Adicionando Registro em ALTERACAO REQUISICAO DE TRANSFERENCIA DE MEDICAMENTOS");
...
if($acao=="edit")
{
    ...
    reglog($id_login,"Editando ALTERACAO REQUISICAO DE TRANSFERENCIA DE MEDICAMENTOS $req_codigo");
...
if($acao=="del")
{
    ...
    reglog($id_login,"Exluindo Registro de ALTERACAO REQUISICAO DE TRANSFERENCIA DE MEDICAMENTOS $req_codigo");
    $stmt = "begin;";
