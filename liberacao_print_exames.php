...
$usu = db_getRow("SELECT usu_datanasc,
						 usu_sexo,
						 usu_cisvir,
						 usu_prontuario,
						 usu_nome,
...
if( empty($imprimir) )
{
	$stmt = "SELECT DISTINCT libexl_dt_cadastro,to_char(libexl_dt_cadastro,'DD/MM/YYYY') as libexl_dt_cadastro from liberacao_exame_lista 
...
<table width='100%' cellspacing='1' cellpadding='1' border='0'>
	<tr>
		<td  width="30"><font size=3 face='courier'>DATA.....:</font></td>
			...
			WHERE al.libex_codigo = $libex_codigo 
			";
...
$stmt = "SELECT ae.libex_codigo,
		  ...
		  WHERE ae.libex_codigo=$libex_codigo";
$row0 = db_getRow( $stmt );
...
<table width='100%' cellspacing='1' cellpadding='1' border='0'>
	<tr>
		<td width=20><font size=3 face='courier'>Cod.Liberacao:</font></td>
...
// sql alterado
// NATURAL JOIN procedimento AS p
$stmt = "SELECT to_char(libexl_dt_cadastro,'dd/mm/yyyy'), TRANSLATE(proc_nome, 'ZZZ-', '') as proc_nome, m.med_nome,p.proc_codigo
FROM liberacao_exame_lista AS al
INNER JOIN procedimento AS p ON al.proc_codigo = p.proc_codigo
LEFT JOIN medico AS m ON p.med_codigo = m.med_codigo
INNER JOIN liberacao_exame AS ae ON ae.libex_codigo = al.libex_codigo
	...
	<td width=65>
		...
		<?php 
			$sqlOrientacoes = "SELECT *
								...
								WHERE proc_codigo = $rr[proc_codigo]";
			$queryOrientacoes = pg_query($sqlOrientacoes);
			echo "<td width=250 align='center'><font size=2 face='courier'>&nbsp;<b>";
			while($regs = pg_fetch_array($queryOrientacoes)){
				echo $regs[ori_exa_orientacoes]."<br/>";				
			}
