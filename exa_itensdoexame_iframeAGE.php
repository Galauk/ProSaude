...
if(!empty($cad_exame)) {
$sel = pg_query("select *from cadastrodoexame where cad_exame = $cad_exame and age_data = '$age_data' and agex_codigo = $agex_codigo and usu_codigo = $usu_codigo") or die(pg_last_error());
if(pg_num_rows($sel)==1) {
 $it = pg_query("select *from itensdoexame where cad_exame = $cad_exame");
if(pg_num_rows($it)==0) {
 $sq = pg_query("select *from agendamento_exame_lista where agex_codigo = $agex_codigo and agexl_data = '$age_data' and usu_codigo = '$usu_codigo' and agexl_status = 'R'") or die(pg_last_error());
while($pr = pg_fetch_array($sq)) {
     $sqnew = pg_query("insert into itensdoexame (cad_exame,proc_codigo,itx_status) values ('$cad_exame','$pr[proc_codigo]','P')") or die(pg_last_error());
   }
  }
 } 
}
...
$rr = pg_fetch_array(pg_query("select TRANSLATE(proc_nome, 'ZZZ-', '') as procnomenew,*from procedimento where proc_codigo = $row[proc_codigo]"));
echo "<tr>
	 <td width=10 align=center>$row[itx_codigo]</td>
	 <td>$rr[procnomenew]</td>
...
if($acao=="add") {
   $sql = pg_query("insert into itensdoexame (cad_exame,proc_codigo,itx_observacao,itx_urgente,itx_status)
        ...
        echo "<SCRIPT LANGUAGE=\"JavaScript\">
                  setTimeout(\"location='exa_itensdoexame_iframeAGE.php?id_login=$id_login&cad_exame=$cad_exame'\", 2000);
              </SCRIPT>";
