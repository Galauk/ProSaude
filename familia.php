...
<script type="text/javascript" src="familia.js"></script>
<?
//------------------------------------------------------------------>
	    ...
	    <legend>Opções</legend>
	     <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
	      <tr>
		   ...
		   {
				echo ChmodBtn($id_login,'microarea','microarea.php?acao=');
		   } else {
				echo "<img src='".$_SESSION[linkroot].$_SESSION[comum]."imgs/microarea_off.jpg' />";
		   }
		   echo "</td>
           <td width=116>";
		   if(SelPerm($id_login,'atendimento_psf.php') != "0")
		   {
				echo ChmodBtn($id_login,'atendimento_psf','atendimento_psf.php?acao=');
		   } else {
				echo "<img src='".$_SESSION[linkroot].$_SESSION[comum]."imgs/atendimento_psf_off.jpg' />";
		   }
		   echo "</td>";
			if(chmodbtn($id_login, "procurar_if", "familia.php"))
			{
  ...
  echo "<form name=familia method=post action=$PHP_SELF>
	    ...
	    <legend>Cadastro de Familia</legend>
	     <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
	      ...
	      <tr>
  		   ...
  		   ?>
		   <td><input type=text name=fam_dt_inclusao value="<?php echo date('d/m/Y'); ?>" class=box size=10></td>
		   <?
	    echo "  </tr>
	      <tr>
	      ...
	      <tr>
		   ...
		   <td>
		   <select name=mic_codigo class=box>";
		   		$sql = "SELECT mic_codigo, mic_descricao FROM microarea";
		   		$exec_sql = pg_query($sql);
		   		while($lista=pg_fetch_array($exec_sql)){
					$mic_codigo = $lista['mic_codigo'];
					$mic_descricao = $lista['mic_descricao'];
					echo "<option value=".$mic_codigo.">".$mic_descricao."</option>";
				}
		   echo "</select>
		   </td>
	      </tr>
	      ...
	      <tr>
		   ...
		   <td>
			<select name=log_tipo class=box>";
	...
	echo "	</select>
	      ...
	      <tr>
		   ...
		   <td>
			<select name=fam_energia class=box>
	      ...
	      <tr>
		   ...
		   <td>
			<select name=fam_esgoto class=box>
	      ...
	      <tr>
		   ...
		   <td>
			<select name=fam_tipo_domicilio class=box>
	      ...
	      <tr>
		   ...
		   <td>
			<select name=fam_agua  class=box>
				<option value=1>Rede P&uacute;blica</option>
	      ...
	      <tr>
		   ...
		   <td>
			<select name=fam_tratamento_agua  class=box>
	      ...
	      <tr>
		   ...
		   <td>
			<select name=fam_destino_lixo  class=box>
	...
	echo "	</select>
	      ...
	      <tr>
		   ...
		   <td>
			...
			<input type=hidden name='cid_codigo' value='$row[cid_codigo]'>
			<a href='javascript:;' onclick=\"mostra_janela('janela_paci');init_paci('<?=$id_login;?>');\">
				<img src='".$_SESSION[linkroot].$_SESSION[comum]."imgs/localizar.jpg' alt='Localizar' align='absmiddle' border='0' />	
			</a>
		   </td>
...
//-> ADD <---------------------------------------------------------->
 ...
 if($acao=="add") {
	$fam_dom_id = $cid_codigo_ibge . $fam_cadastrador . $fam_nr_ficha;
	$dv = ($fam_dom_id - 11) * round($fam_dom_id/11);
	$fam_dom_id = $cid_codigo_ibge . $fam_cadastrador . $fam_nr_ficha . $dv;
