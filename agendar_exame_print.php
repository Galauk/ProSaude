...
$stmt = "SELECT m.med_nome, 
				m.med_end_telefone, 
				m.med_endereco,
				m.med_end_numero, 
				r.rua_nome,
				c.cid_nome
		   FROM agendamento_exame AS ae
		  ...
		  WHERE ae.agex_codigo=$agex_codigo";
$row0 = db_getRow( $stmt );
...
<table width='100%' cellspacing='1' cellpadding='1' border='0'>
	...
	<tr>
		...
		<td><font size=3 face='courier'>&nbsp;<b><? echo $row0[2].",". $row0[3];?></b></font></td> 
	</tr>
	<tr>
		 <td width=65><font size=3 face='courier'>CIDADE...:</font></td>
		 <td><font size=3 face='courier'>&nbsp;<b><?=$row0[5]?></b></font></td>
		 <td width=65><font size=3 face='courier'>TELEFONE...:</font></td>
		 <td><font size=3 face='courier'>&nbsp;<b><?=$row0[1]?></b></font></td>
	</tr>
</table><br>
...
// sql alterado
// NATURAL JOIN procedimento AS p
$stmt = "SELECT to_char(agexl_data,'dd/mm/yyyy'), TRANSLATE(proc_nome, 'ZZZ-', '') as proc_nome, m.med_nome,p.proc_codigo
FROM agendamento_exame_lista AS al
INNER JOIN procedimento AS p ON al.proc_codigo = p.proc_codigo
LEFT JOIN medico AS m ON p.med_codigo = m.med_codigo
INNER JOIN agendamento_exame AS ae ON ae.agex_codigo = al.agex_codigo
