	...
	if($acao == "acao"){
		...
		$sql = "SELECT *
				  ...
				  JOIN procedimento as p
				    ON p.proc_codigo = po.proc_codigo
				  JOIN orientacoes_exames as oe
				    ON oe.ori_exa_codigo = po.ori_exa_codigo
				 WHERE po.proc_codigo = $proc_codigo";
		$query = pg_query($sql);
		$numLinhas = pg_num_rows($query);
		
		echo $table->openTable("lista");
		echo $common->divisoria("Orienta&ccedil;&otilde;es do exame <font color='#FFFFFF'><em> $proc_nome </em></font>");
		echo $table->criaLinha(array("C&oacute;digo Procedimento","Orienta&ccedil;&otilde;es","A&ccedil;&otilde;es"),null,null,"S");
		while($res = pg_fetch_array($query)){
			echo $table->criaLinha(array($res[proc_ori_codigo],
										 $res[ori_exa_orientacoes],
										 $common->commonButton("apagar",null,"apagar.png","onClick=\"chamaVinculacoes('$res[proc_ori_codigo]','$res[proc_nome]','acao','$res[proc_codigo]')\"")));
		}
