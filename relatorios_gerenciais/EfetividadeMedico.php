...
echo "
  <form method=post action=$PHP_SELF>
    ...
    <tr>
               ...
               <td>
                   <select name=esp_codigo class=boxr>";
                   ...
                   while($esp=pg_fetch_array($sql)) {
				   		if($_REQUEST['esp_codigo'] == $esp['esp_codigo']){
...
//vSQL($sql,"1");
echo "asdfasdf:".pg_num_rows($sql)."<br>".$sql;
$query=pg_query($sql);
...
//----------------  Rotina de Impressão  ---------------->
  ...
  while($row=pg_fetch_row($query)) {
     ...
     if ($esp_codigo) {
         $sqlCount.= " AND agendamento.esp_codigo = $esp_codigo                     "; }
