...
$titulo="Quantidade de Consultas Agendadas por Paciente";    //       NOME DO RELATÓRIO
...
echo "
  <form method=post action=$PHP_SELF>
    ...
    <tr>
	     ...
	     <tr>
		    <td width=110>Qtde Consultas:</td>
		    <td>
			<input type=text name=qtde class=box size=13 value=$qtde>
			</td>
	     </tr>";
...
//----------------  Cabeçalho dos Dados  ------------------>
       ...
       if ($Cab == 0) {
           echo " <tr>\n";
           echo "  <td width=40% style='font-weight:bold'>Paciente</td>\n";
           ...
           echo "  <td width=12% style='font-weight:bold' align=center>Nao Atend</td>\n";
           echo "  <td width=12% style='font-weight:bold' align=center>Faltas</td>\n";
           echo "  <td width=12% style='font-weight:bold' align=center>Transf</td>\n";
           echo " </tr>\n";
        }
}
...
$sql = "SELECT DISTINCT age.usu_codigo, usuario.usu_nome
        FROM  agendamento as age
        INNER JOIN usuario
        ON age.usu_codigo = usuario.usu_codigo
        WHERE age.age_data between '$dt_inicial' AND '$dt_final'
        AND (select count(age_codigo) from agendamento
             where usu_codigo = age.usu_codigo and age_data between '$dt_inicial' AND '$dt_final') > '$qtde'";

if ($uni_codigo) {
       $sql .= "   AND agendamento.uni_codigo = $uni_codigo                "; }

$sql .=" ORDER BY usuario.usu_nome";
...
//----------------  Rotina de Impressão  ---------------->
  ...
  while($row=pg_fetch_row($query)) {
     ...
     $sqlCount = "SELECT  count(age_codigo) as AgendAgend, " .
                 ...
                 "  FROM  agendamento                                               " .
                 " WHERE  agendamento.usu_codigo = $row[0]                          ";
     if ($uni_codigo) {
...
//  echo "  <td align=right>TOTAL  DE ATENDIDOS -&nbsp;&nbsp;&nbsp;&nbsp;$TotAtendidos</td>\n";
           ...
           echo "</tr>";
           echo " <tr>\n";
           echo "  <td  style='font-weight:bold'> Total de Pacientes com mais de $qtde Agendamentos = $pacientes</td>\n";
