<?php
	...
	{		
		if($acao == "buscar_nome")
		{
			$where = " usu_nome like upper('%".trim($palavra)."%')and usu_ativacao = 'S' ";
		} else if($acao == "buscar_data") {
			$where = " usu_datanasc = '".trim($palavra)."%'and usu_ativacao = 'S' ";
		}
	...
	while($row = pg_fetch_array($exec))
	{
		...
		echo "
			<tr id='tr$i' onmouseover=\"trocar_cor(this.id, 'fone$i');\" onmouseout=\"retirar_cor(this.id, 'fone$i')\"
				onclick=\"passar_usuario($row[usu_codigo], '$row[usu_nome]', '$row[usu_mae]', '$row[usu_datanasc]', '$aux', '$row[usu_prontuario]','$farmacia')\" style='cursor:pointer;'>
				<td>
					$row[usu_prontuario]
				</td>
				<td>
