...
//log
reglog($id_login,"Acessando o programa dos números de APAC");

// busca
	echo "
	<form action='#' method='get' onsubmit='return busca_numeros(\"$id_login\",\"$tipo\");'>
    	<fieldset>
      	...
      	<tr>
      		...
      		<td width=120>
      			...
      			<input type='text' name='palavra_chave' id='num_palavra_chave' class=box
...
if( empty($acao) || $acao == 'busca' )
{
	...
	$resp 	= '';
	$max 	= 10;
	$AND = empty($acao) ? '' : ("AND aan_numero_resto = '".$str."' ");
	//$AND = empty($acao) ? '' : sprintf("AND aan_numero_resto ILIKE '%%%s%%'", $str);
...
// 		ESQ_SAUDE, $where1, ESQ_SAUDE, $where2, $sql_f );
   ...
   $stmt = "SELECT aan_codigo, aan_numero_resto, aan_tipo from aih_apac_numeros_resto where aan_tipo='APAC' ". $AND ."". $sql_f ;
 ...
 */
	...
	<tr bgcolor='#ffffff'>
		...
		<th>Número</th>
		<th style='text-align:center' width='100'>Tipo</th>
		<th>&nbsp;</th>
	</tr>
	";
	while($row=pg_fetch_array($qry))
	{
       echo "
	         ...
	         <td width='230' class='c'>
    				<a href='javascript:;'
    					onclick=\"add_numero('$row[0]','$row[1]');\">
    					...
    					<img src='".$_SESSION[linkroot].$_SESSION[comum]."imgs/apagar_on.jpg' border='0' alt='Apagar' style='cursor:pointer;'
    						onclick=\"apagar_numero('$tipo','$row[0]');\" />
			   </td>
	     </tr>";
     }
	echo "
...
{
	$stmt = sprintf("INSERT INTO %s.apac_unidade
		(uni_desc, uni_localizacao, uni_responsavel, uni_cnpj) VALUES
		('%s', '%s', '%s', '%s')",
...
{
	$stmt = sprintf("UPDATE %s.apac_unidade SET
		uni_desc = '%s',
		uni_localizacao = '%s',
		uni_responsavel = '%s',
		uni_cnpj = '%s'
...
if( $acao == 'del' )
{
	...
	reglog($id_login,"Deletando o número de APAC");

	$stmt = "DELETE FROM aih_apac_numeros_resto WHERE aan_codigo = ".$codigo;
	db_query( $stmt );

	print "<p class='aviso'>Número Removido</p>";
