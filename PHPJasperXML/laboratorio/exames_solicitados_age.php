...
$usr_codigo = $_POST["usr_codigo_solicitante"];
...
if($usr_codigo){
	if($interno == 1)
		$where = " AND usr_codigo_medico = $usr_codigo";
	else{
		$where = " AND a.med_codigo = $usr_codigo";
	}
}
	
if($data_inicial)
	$where .= " AND age_data_insert >= '$data_inicial'";

if($data_final) 
	$where .= " AND age_data_insert <= '$data_final'";
...
$sql = " select p.proc_codigo, 
		   ...
		   left join usuarios u
		     on u.usr_codigo=a.usr_codigo_medico
		  where 1=1
		  $where
		 group by p.proc_codigo,p.proc_nome,usr_nome order by usr_nome,count desc";
//die($sql);
...
$PHPJasperXML->load_xml_file("exames_solicitados_age.jrxml");
