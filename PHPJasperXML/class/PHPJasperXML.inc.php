...
class PHPJasperXML {
    ...
    public function connect($db_host,$db_user,$db_pass,$db_or_dsn_name,$pg_port,$cndriver="mysql") {
    $this->db_host=$db_host;
	$this->db_user=$db_user;
    $this->db_pass=$db_pass;
    $this->db_or_dsn_name=$db_or_dsn_name;
    $this->cndriver=$cndriver;
	$this->db_port=$pg_port;
        if($cndriver=="mysql") {
			...
			$port = $pg_port;
			//die("-".$port."-".$pg_port."-");
            //$conn_string = "host=$db_host port=$pg_port dbname=$db_or_dsn_name user=$db_user password=$db_pass";
			//die("host=$db_host port=$port dbname=$db_or_dsn_name user=$db_user password=$db_pass");
			$this->myconn = pg_connect("host=$db_host port=$pg_port dbname=$db_or_dsn_name user=$db_user password=$db_pass") or die("Falha na conexão");
			//die($conn_string);
            //$this->myconn = pg_connect("host=$db_host port=$pgport dbname=$db_or_dsn_name user=$db_user password=$db_pass") or die("mdf");
...
//                $arrdata[$num]=str_replace('"',"",$out);
    ...
    public function includeSubReport($d,$arrdata,$current_y){ 
              ...
              $this->passAllArrayDatatoSubReport($PHPJasperXMLSubReport,$d,$current_y,$arrdata);
               ...
               $PHPJasperXMLSubReport->outpage("I");    //page output method I:standard output  D:Download file
