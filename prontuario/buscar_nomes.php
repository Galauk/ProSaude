<?php
	...
	{		
		...
		$sql =  "SELECT usu_codigo,
						...
						muni_cd_cod_ibge_resid, 
						usu_fone, 
						usu_celular, 
						usu_cpf, 
						usu_rg,
						CASE 
							WHEN usu_cartao_sus is NOT null 
							THEN usu_cartao_sus 
						ELSE 
					    usu_cartao_p_sus end AS usu_cartao_sus
				   FROM usuario
			      WHERE $where 
				  ORDER BY usu_nome $limit";
	...
	while($row = pg_fetch_array($exec))
	{
		...
		/*$resp .= "[{\"codigo\" : $row[usu_prontuario], \"nome\" : \"$row[usu_nome]\", \"mae\" : \"$row[usu_mae]\", \"data_nasc\" : \"$row[usu_datanasc]\", \"cidade\" : \"$aux\"}]";
