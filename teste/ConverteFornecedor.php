...
<? include '../global.php';
	...
	$sql = "select distinct forn_desc from ofertas_solicitacoes order by forn_desc";
	$query = pg_query($sql);
	$cont = 1;
		...
		<tr>
				...
				for ($i = 0; $i<count($resul);$i++){
					if(!is_numeric($resul[$i])){
						if(!in_array($resul[$i], $array)){
							...
							$select2  = "SELECT * FROM fornecedor where for_nome ilike '%$resul[$i]%'";
							//echo $select2."<br>";
							...
							while ($res2 = pg_fetch_array($query2)){
								...
								//echo $res2[produto]."<br>";
									array_push($produto, $res2[for_nome]."==".$res2[for_codigo]);
			...
			//	for ($i = 0; $i<count($produtoUnicos);$i++){
				...
				foreach ($produtoUnicos as $cod => $item){
					...
					echo"</td>
						<td>
							UPDATE ofertas_solicitacoes set for_codigo = $separa[1] where forn_desc = '$r[forn_desc]';
						</td>
					</tr>";				
