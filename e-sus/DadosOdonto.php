...
class ThriftOdonto {
	...
	public function fichaAtendimentoOdontologicoChildThrift($row){
		$array_atend_odonto = array(
			...
			"numCartaoSus" => ($this->validaCnsBanco(trim($row['eo_num_cartao_sus']))==true ? trim($row['eo_num_cartao_sus']) : NULL),
			"numProntuario" => $row['eo_numprontuario'],
			"localAtendimento" => $row['co_local_atend']." L",
			"tipoAtendimento" => $row['eo_tipo_atendimento']." L",
			"tiposEncamOdonto" => $this->getCondutaEncaminhamento($row['odo_pcon_codigo']),
			"tiposFornecimOdonto" => array(),
			"tiposVigilanciaSaudeBucal" => $this->getVigilanciaSaudeBucal($row['odo_pcon_codigo']),
			"tiposConsultaOdonto" => ($row['eo_tipo_consulta'] != "" ? array($row['eo_tipo_consulta']." L") : array()),
			"procedimentosRealizados" => array(),
			"outrosSiaProcedimentos" => $this->listaProcedimentosSigtap($row['odo_pcon_codigo']),
			"sexo" => $row["eo_sexo"]
		);
		//echo"<pre>".print_r($array_atend_odonto,1);
	...
	public function listaProcedimentosSigtap($odoPconCodigo){
		...
		foreach($dadosProcedimentosSigtap as $val) {
			$array = array(
				"coMsProcedimento" => $val["proc_codigo_sus"],
				"quantidade" => "1"
			);
	...
	public function getCondutaEncaminhamento($odoPconCodigo){
		...
		foreach($dadosEncaminhamento as $rowSql) {
			array_push($procsRea, $rowSql["tp_cds_encam_odonto"]." L");
		}
		return $procsRea;
	}
	...
	public function getVigilanciaSaudeBucal($odoPconCodigo){
		...
		foreach($dadosVigilancia as $rowSql) {
			array_push($procsRea, $rowSql["tp_cds_vig_saude_bucal"]." L");
		}
		return $procsRea;
	}
