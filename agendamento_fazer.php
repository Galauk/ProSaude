...
<script> 
	function salvarLista() { 
		pac_codigo = document.getElementById('usu_codigo').value;
		...
		url = "salvarListaEspera.php?usu_codigo="+pac_codigo+"&esp_codigo="+esp_codigo+"&med_codigo="+med_codigo+"&uni_codigo="+uni_codigo+"&agt_codigo="+agt_codigo+"&id_login="+id_login; 
		ajax = ajaxInit();
...
<?php
	...
	echo "
		<form method='post' action='agendamento_atendimento.php' target='iframe_dir'>
			...
			<input type='hidden' name='esp_codigo' id='esp_codigo' value='$esp_codigo'>
			<input type='hidden' name='age_item' id='age_item' value='$age_item'>
			<input type='hidden' name='uni_codigo' id='uni_codigo' value='$uni_codigo'>
			<input type='hidden' name='med_codigo' id='med_codigo' value='$med_codigo'>
			<input type='hidden' name='agt_codigo' id='agt_codigo' value='$agt_codigo'>
			<input type='hidden' name='age_tipo' id='age_tipo' value='$age_tipo'>
			<input type='hidden' name='age_vaga' id='age_vaga' value='$age_vaga'>
			<input type='hidden' name='usu_codigo' id='usu_codigo' value='$usu_codigo'>
			<input type='hidden' name='age_paciente' id='age_paciente' value='$age_paciente'>
			<input type='hidden' name='acao' id='acao' value='addagendamento'>
			<input type='hidden' name='grm_mensal' id='grm_mensal' value='$grm_mensal[grm_qtde]'>
			<table width='100%' cellspacing='1' cellpadding='4' border='0' class='lista'>
				<tr>
                    <th width='10'>&nbsp;</th>
                    <th width='10'>Tp.Age.</th>
		...
		echo "
			<tr bgcolor='#FFFFFF' id='mensagem'>
				...
				if($age_item == "ES")
				{
					echo "
						<td>
							<input type='image' src='".$_SESSION[linkroot].$_SESSION[comum]."imgs/lista_espera_on.jpg' onclick='return salvarLista();'>
						</td>";
				}
		echo "</tr>";
		exit;
	...
	if(!empty($row[gra_data]))
	{
		echo "
			...
			<td width='10' align=center>$age_tipo</td>
	...
	if(!empty($row["gra_data"]))
	{
		echo "<td width='90'>$dia_da_semana</td>";
	}
