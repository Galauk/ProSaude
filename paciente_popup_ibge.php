...
// busca
	echo "
	<form action='#' method='get' onsubmit='return busca_ibge_nasc(\"$id_login\");'>
	<fieldset>
...
// 	$where1 = empty($acao) ? '' : sprintf("AND cid_nome ILIKE '%%%s%%'", $str);
	...
	if (empty($where1)){
		$stmt = "SELECT  cid_nome, cid_codigo_ibge FROM cidade ORDER BY 2 ". $sql_f ;
	}else{	
		$stmt = "SELECT  cid_nome, cid_codigo_ibge FROM cidade ". $where1 ;
		
	}
	//." ORDER BY 2 ". $sql_f ; FIM DO SQL ACIMA QUE DAVA ERRO NA FUNCAO
	...
	while($row=pg_fetch_array($qry))
	{
       echo "
	        ...
	        <td width='230' class='c'>
				<a href='javascript:;'
					onclick=\"add_cod_ibge_nasc('$row[1]');\">
