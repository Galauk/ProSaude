	   ...
	   <fieldset>
	    <legend>Opções</legend>
	     <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
	      <tr>
	       <form method=post action='list_pacientes.php?$QUERY_STRING'>
  ...
  echo "<table width=98% align=center cellspacing=0 cellpadding=0 border=0>
	 <tr>
	  <td>
   	...
   	$sql=db_query("select usu_prontuario,usu_codigo,usu_nome,to_char(usu_datanasc,'DD/MM/YYYY') as usu_datanasc,usu_mae,usu_sexo,usu_end_cidade,usu_ativacao from usuario where usu_ativacao='S'order by usu_codigo desc limit 15");
     while($row=pg_fetch_array($sql)) {
       echo "<tr>
	       ...
	       <td width=60 style='border-bottom:1px dotted;border-right:1px dotted;border-color:c9c9c9;'><a href=\"javascript: getpaciente('$row[usu_codigo]','$row[usu_nome]','$row[usu_datanasc]','$row[usu_mae]','$row[usu_end_cidade]');\"><img src=".$_SESSION[linkroot].$_SESSION[comum]."imgs/selecionar_on.jpg border=0></a></td>
	     </tr>";
     }
	echo "</tr>
	     </table>
	  ...
	  <td>
	   <fieldset>
	    <legend>Opções</legend>
	     <table width=100% align=center cellspacing=3 cellpadding=0 border=0>
	      <tr>
	       <form method=post action='paciente_medicamentos.php?$QUERY_STRING'>
		<input type=hidden name=acao value=busca>
	<input type=hidden name=id_login value=$id_login>
		<input type=hidden name=id_login value=$id_login>
	       <td width=30>Buscar:</td>
	       ...
	       <td>".ChmodBtn($id_login,'procurar','paciente_medicamentos.php')."</td></form>
	      </tr>
	     </table>
	   </fieldset>
	  </td>
...
if($tpbusca=="dt") {
   $sql=db_query("select usu_prontuario,usu_codigo,usu_nome,to_char(usu_datanasc,'DD/MM/YYYY') as usu_datanasc,usu_mae,usu_sexo,usu_ativacao from usuario where to_char(usu_datanasc,'DD/MM/YYYY')='$palavra_chave' and usu_ativacao = 'S' order by usu_nome");
}

if($tpbusca=="sbr") {
   $sql=db_query("select usu_prontuario,usu_codigo,usu_nome,to_char(usu_datanasc,'DD/MM/YYYY') as usu_datanasc,usu_mae,usu_sexo,usu_ativacao from usuario where usu_nome like '%$palavra_chave' and usu_ativacao = 'S' order by usu_nome");
}

if($tpbusca=="pr") {
   $sql=db_query("select usu_prontuario,usu_codigo,usu_nome,to_char(usu_datanasc,'DD/MM/YYYY') as usu_datanasc,usu_mae,usu_sexo,usu_ativacao from usuario where usu_prontuario like '%$palavra_chave' and usu_ativacao = 'S' order by usu_prontuario");
}
...
if($tpbusca=="nm") {
   $sql=db_query("select usu_prontuario,usu_codigo,usu_nome,to_char(usu_datanasc,'DD/MM/YYYY') as usu_datanasc,usu_mae,usu_sexo,usu_ativacao from usuario where usu_mae like '$palavra_chave%' and usu_ativacao = 'S' order by usu_mae");
}
